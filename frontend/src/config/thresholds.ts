...
export function exceedsAlertThresholds(
  ...
  const percentageThreshold =
    customThresholds?.percentage ?? PRICE_ALERT_THRESHOLDS.DEFAULT_PERCENTAGE;
  const absoluteThreshold = customThresholds?.absolute ?? PRICE_ALERT_THRESHOLDS.DEFAULT_ABSOLUTE;

  return (
    absoluteChange > 0 &&
    (percentageChange > percentageThreshold || absoluteChange > absoluteThreshold)
  );
}
