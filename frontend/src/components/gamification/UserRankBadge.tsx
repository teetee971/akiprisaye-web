...
export function UserRankBadge({ rank, size = 'md', showRank = false, className = '' }: UserRankBadgeProps) {
  const config = rankConfig[rank as keyof typeof rankConfig];
  const sizeConfig = sizeClasses[size];

  if (!config) {
    return showRank ? (
      <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full bg-gray-100 text-gray-700 text-sm ${className}`}>
