  ...
  return (
    <div className="bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/30 rounded-xl p-6 mb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xl font-bold text-gray-100">📊 Résumé de la comparaison</h2>
      </div>
      ...
      <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700/50">
        <p className="text-sm text-gray-300 leading-relaxed">
          💡 <strong className="text-blue-400">Conseil</strong> : En choisissant{' '}
          <strong className="text-green-400">{bestProvider}</strong> au lieu de l'offre la plus
          chère, vous pouvez économiser jusqu'à{' '}
          <strong className="text-orange-400">{formatPrice(savingsAmount)}</strong> (
          {savingsPercentage.toFixed(1)}%).
          {bestTiming &&
            ` Réservez idéalement ${bestTiming.daysRange.toLowerCase()} pour les meilleurs prix.`}
        </p>
      </div>
    </div>
  );
