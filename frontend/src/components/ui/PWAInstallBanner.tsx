...
export function PWAInstallBanner() {
  ...
  useEffect(() => {
    const stored = (window as Window & { __pwaInstallPrompt?: BeforeInstallPromptEvent }).__pwaInstallPrompt;
    if (stored) {
      setPrompt(stored);
    }
    ...
    const handler = (e: Event) => {
      ...
      (window as Window & { __pwaInstallPrompt?: BeforeInstallPromptEvent }).__pwaInstallPrompt = promptEvent;
  ...
  return (
    <div className='fixed bottom-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-96 z-50 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xl p-4 flex items-center gap-3'>
      ...
      <div className='flex-1 min-w-0'>
        <p className='text-sm font-semibold text-gray-900 dark:text-white'>Installer A KI PRI SA YÉ</p>
        <p className='text-xs text-gray-500 dark:text-gray-400'>Accès rapide, fonctionne hors-ligne</p>
      </div>
