      ...
      <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border ${colorClass}`}>
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm opacity-75">{reliability.score}/100</span>
          </div>
          <div className="flex items-center gap-2 text-xs opacity-75">
            <span>
              {reliability.confirmations} confirmation{reliability.confirmations > 1 ? 's' : ''}
            </span>
            {reliability.verifiedBy.length > 0 && (
              <>
                <span>•</span>
                <span>
                  {reliability.verifiedBy.length} source
                  {reliability.verifiedBy.length > 1 ? 's' : ''}
                </span>
              </>
            )}
          </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex flex-wrap gap-1">
              {reliability.verifiedBy.map((verifier) => (
                <span key={verifier} className="px-2 py-0.5 bg-gray-100 rounded text-xs">
