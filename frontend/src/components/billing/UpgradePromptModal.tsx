  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 p-5">
        <h3 id="upgrade-modal-title" className="mb-2 text-lg font-semibold text-white">Fonctionnalité limitée</h3>
        <p className="mb-4 text-sm text-slate-300">{detail.message ?? 'Cette action nécessite un plan supérieur.'}</p>
        <div className="flex justify-end gap-3">
          <button ref={closeRef} className="rounded bg-slate-700 px-4 py-2 text-sm" onClick={() => setDetail(null)}>Fermer</button>
