...
import {
  getCheapestProductsCount,
  getCheapestProductsByStore,
...
export function StoreRanking() {
  ...
  const rankings: StoreRank[] = useMemo(() => {
    ...
    const storesWithCounts = territoryStores
      .map((store) => ({
        storeId: store.id,
        name: store.name,
        chain: store.chain,
  ...
  return (
    <>
      <GlassCard className="bg-yellow-900/10 border-yellow-500/30">
        <div className="space-y-4">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400">Cliquez sur une enseigne pour voir les détails</p>
          </div>
              ...
              rankings.map((store) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-end">
                    <div className="text-2xl font-bold text-green-400">#{store.rank}</div>
                    <div className="text-xs text-blue-400 mt-1">👉 Voir détails</div>
                  </div>
                </button>
              ))
            )}
          ...
          <div className="text-center pt-4 border-t border-slate-700">
            <div className="text-xs text-gray-400">📊 Données publiques – observatoire citoyen</div>
      ...
      {/* Detail Panel Modal */}
      {selectedStore && (
        <StoreCheapestProductsPanel data={selectedStore} onClose={() => setSelectedStore(null)} />
      )}
    </>
  );
}
