...
export function PriceAnomalyBadge({
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-3 py-2 bg-red-900/30 border border-red-500/50 rounded-lg ${className}`}
    >
      <span className="text-xl">⚠️</span>
      <div className="text-left">
        <div className="text-sm font-bold text-red-300">Prix inhabituellement élevé</div>
        <div className="text-xs text-red-400">
          +{percentageAboveAverage}% au-dessus de la moyenne locale
        </div>
        {showTooltip && (
...
export function ProductCardWithAnomaly() {
  return (
    <div className="p-4 bg-slate-800/50 rounded-lg space-y-3">
      ...
      <div className="text-xs text-gray-400">Prix moyen observé : 2,99 €</div>
    </div>
  );
}
