...
const SAMPLE_ANNONCES: AideAnnonce[] = [
  {
    ...
    description:
      'Jardinier amateur expérimenté, je peux aider pour taille de haies, désherbage et plantation. Matériel disponible.',
  ...
  },
  {
    ...
    description:
      "Fuite sous l'évier de cuisine. Je peux fournir les pièces si quelqu'un peut m'aider à faire la réparation.",
  ...
  },
  {
    ...
    description:
      'Disponible 2 fois par semaine pour faire les courses pour personnes ne pouvant pas se déplacer.',
  ...
  },
  {
    ...
    description:
      "Je propose d'aider les personnes âgées à utiliser leur smartphone, tablette ou ordinateur. Patience garantie !",
  ...
  },
  {
    ...
    description:
      'Personne à mobilité réduite cherche covoiturage pour rendez-vous hospitalier le matin.',
  ...
  },
  {
    ...
    description:
      'Je cuisine souvent en grande quantité, je propose de partager des plats créoles faits maison avec les voisins dans le besoin.',
...
function PublishForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      <div className="grid grid-cols-2 gap-3">
        {(['offre', 'demande'] as AideType[]).map((t) => (
          ...
          >
            {t === 'offre' ? (
              <>
                <HandHeart className="w-4 h-4" /> Je propose mon aide
              </>
            ) : (
              <>
      ...
      {/* Catégorie */}
      <div>
        <label htmlFor="aide-category" className="block text-sm font-medium text-gray-300 mb-1">
          Catégorie *
        </label>
        <select
          id="aide-category"
          value={category}
          onChange={(e) => setCategory(e.target.value as AideCategory)}
      ...
      {/* Titre */}
      <div>
        <label htmlFor="aide-titre" className="block text-sm font-medium text-gray-300 mb-1">
          Titre *
        </label>
        <input
          id="aide-titre"
          ...
          placeholder={
            type === 'offre'
              ? 'Ex: Je propose mon aide pour le jardinage'
              : "Ex: Besoin d'aide pour peindre mon salon"
          }
      ...
      <div>
        <label
          htmlFor="entraide-description"
          className="block text-sm font-medium text-gray-300 mb-1"
        >
          Description *
        </label>
        <textarea
          id="entraide-description"
          name="description"
          value={description}
      ...
      <div>
        <label
          htmlFor="aide-disponibilite"
          className="block text-sm font-medium text-gray-300 mb-1"
        >
          Disponibilité *
        </label>
        <input
          id="aide-disponibilite"
          type="text"
          value={disponibilite}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label htmlFor="aide-territoire" className="block text-sm font-medium text-gray-300 mb-1">
            Territoire *
          </label>
          <select
            id="aide-territoire"
        ...
        <div>
          <label htmlFor="aide-quartier" className="block text-sm font-medium text-gray-300 mb-1">
            Quartier / Ville
          </label>
          <input
            id="aide-quartier"
...
function AnnonceCard({
  ...
  const dateStr = new Date(annonce.createdAt).toLocaleDateString('fr-FR', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
  });
      ...
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <span
              className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isOffre ? 'bg-green-600/20 text-green-300' : 'bg-orange-600/20 text-orange-300'}`}
            >
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Proposez votre aide ou demandez un coup de main à votre communauté
            </p>
          </div>
          <button
      ...
      {justPublished && (
        <div className="flex items-center gap-2 bg-green-900/30 border border-green-600/40 rounded-xl px-4 py-3">
          ...
          <p className="text-sm text-green-300">
            Annonce publiée ! Elle est visible par votre communauté.
          </p>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <select
          id="entraide-filter-type"
          name="filterType"
          value={filterType}
          onChange={(e) => setFilterType(e.target.value as AideType | 'all')}
        ...
        <div className="space-y-3">
          {filtered.map((a) => (
            <AnnonceCard
              key={a.id}
              annonce={a}
              onDelete={isUserAnnonce(a.id) ? handleDelete : undefined}
            />
          ))}
        </div>
      )}
      ...
      <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-4">
        ...
        <ul className="text-xs text-gray-400 space-y-1">
          <li>
            • <strong className="text-gray-300">Offre d'aide</strong> : proposez bénévolement votre
            temps et vos compétences
          </li>
          <li>
            • <strong className="text-gray-300">Demande d'aide</strong> : sollicitez un coup de main
            ponctuel
          </li>
