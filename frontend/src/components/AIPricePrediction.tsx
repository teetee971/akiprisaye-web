...
export default function AIPricePrediction({
  ...
  const bandColor = spread < 5 ? 'bg-emerald-500' : spread <= 15 ? 'bg-yellow-400' : 'bg-rose-500';
  const bandLabel =
    spread < 5 ? 'text-emerald-300' : spread <= 15 ? 'text-yellow-300' : 'text-rose-300';
        ...
        <div className="bg-amber-900/20 border border-amber-700/30 rounded-lg p-3 flex items-start gap-2">
          ...
          <div className="text-xs text-amber-200/90 leading-relaxed">
            <p className="font-medium mb-1">
              ⚠️ Aucune recommandation d'achat – usage informatif uniquement
            </p>
            <p className="text-amber-300/70">
              Ce n'est pas une certitude, mais une projection factuelle basée sur l'analyse de
              données publiques historiques. Les prix réels peuvent varier selon de multiples
              facteurs.
            </p>
          </div>
        </div>
