  ...
  return (
    <div className="space-y-4" role="region" aria-label="Statistiques de courses">
      ...
      <div className="bg-gradient-to-br from-purple-900/30 to-blue-900/30 border border-purple-700/30 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 id="stats-title" className="text-lg font-semibold text-purple-300">
              📊 Vos Statistiques
            </h3>
          </div>
          {onClearStats && (
        ...
        <div
          className="mt-3 p-2 bg-emerald-900/20 rounded border border-emerald-700/30"
          role="note"
        >
          <div className="text-xs text-emerald-200">
            🌱 Équivalent à <strong>{Math.round(stats.co2Saved / 2.3)} km</strong> en voiture non
            parcourus
          </div>
        </div>
      </div>
      ...
      {unlockedBadges.length > 0 && (
        <div
          className="bg-gradient-to-br from-yellow-900/30 to-orange-900/30 border border-yellow-700/30 rounded-lg p-4"
          role="region"
          aria-label="Badges débloqués"
        >
          <div className="flex items-center gap-2 mb-3">
            <Award className="w-5 h-5 text-yellow-400" aria-hidden="true" />
            <h3 id="badges-title" className="text-lg font-semibold text-yellow-300">
              🏆 Badges Débloqués
            </h3>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2" role="list" aria-labelledby="badges-title">
            {unlockedBadges.map((badge) => (
              ...
              >
                ...
                <div className="text-xs text-center text-gray-300 font-medium">{badge.name}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {nextBadges.length > 0 && (
        <div
          ...
          role="region"
          aria-label="Prochains objectifs"
        >
          <div className="text-sm font-semibold text-gray-300 mb-3">🎯 Prochains objectifs</div>
                ...
                <div key={badge.id} className="space-y-1" role="listitem">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span
                      ...
                      aria-label={`Progression : ${progress.toFixed(1)} sur ${target}`}
                    >
                  ...
                  <div
                    ...
                    aria-valuemin={0}
                    aria-valuemax={100}
                    aria-label={`${badge.name} : ${percentage.toFixed(0)}% complété`}
                  >
                    <div
      ...
      <div className="p-3 bg-slate-800/20 border border-slate-700/30 rounded-lg" role="note">
        <div className="text-xs text-gray-400">
          🔒 <strong>Confidentialité :</strong> Toutes vos statistiques sont stockées localement sur
          votre appareil. Aucune donnée n'est envoyée à nos serveurs.
        </div>
      </div>
    </div>
  );
