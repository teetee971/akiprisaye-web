  ...
  return (
    <div className="scan-pwa-card">
      ...
      <section className="pwa-info">
        <img src={product.image} alt={product.name} width={96} height={96} className="pwa-img"/>
        <div>
          <h2>{product.name}</h2>
      ...
      <div className="pwa-cards">
        {stores.map((store) => (
          <div key={`${store.brand}-${store.store}-${store.territory}`} className="pwa-card" title={`Top ${store.rank} territoire`}>
            ...
            <img src={store.logo} alt={store.brand} width={40} height={40} className="pwa-card-logo"/>
            <div>{store.brand}</div>
