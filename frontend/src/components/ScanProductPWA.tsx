...
export default function ScanProductPWA({
  product,
  stores,
  france,
  history,
  ...
  return (
    <div className="scan-pwa-card">
      <header className="pwa-header">
        <button className="pwa-back" aria-label="Retour">
          <span aria-hidden="true">←</span>
        </button>
        <span className="pwa-title">Produit scanné</span>
        <button className="pwa-share" aria-label="Partager">
          <span aria-hidden="true">🔗</span>
        </button>
      </header>
      ...
      <div className="pwa-cards">
        {stores.map((store) => (
          <div
            key={`${store.brand}-${store.store}-${store.territory}`}
            className="pwa-card"
            title={`Top ${store.rank} territoire`}
          >
            ...
            <img
              ...
              height={40}
              loading="lazy"
              className="pwa-card-logo"
            />
            <div>{store.brand}</div>
            <div className="pwa-card-price">{store.price.toFixed(2)} €</div>
            <div className="pwa-card-detail">
              {store.store} • {store.distance}
            </div>
      ...
      <section className="pwa-insights">
        {insights.map((ins) => (
          <span key={ins} className="pwa-insight">
            {ins}
          </span>
        ))}
      </section>
      <section className="pwa-actions">
        {features.map((f) => (
          <button key={f.label} className="pwa-feature">
            {f.icon} {f.label}
          </button>
        ))}
      </section>
