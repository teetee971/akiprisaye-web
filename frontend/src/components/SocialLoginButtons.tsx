...
const FacebookLogo = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
    <path
      fill="#1877F2"
      d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.41c0-3.025 1.792-4.696 4.533-4.696 1.313 0 2.686.236 2.686.236v2.97h-1.513c-1.491 0-1.956.93-1.956 1.886v2.267h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z"
    />
  </svg>
);
...
export default function SocialLoginButtons({
  ...
  const { signInGooglePopup, signInFacebookPopup, signInApplePopup, user } = useAuth();
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col gap-2.5">
        ...
        >
          {busy === 'google' ? (
            <span
              className="w-4.5 h-4.5 border-2 border-slate-300 border-t-blue-500 rounded-full animate-spin inline-block"
        ...
        >
          {busy === 'facebook' ? (
            <span
              className="border-2 border-blue-200 border-t-white rounded-full animate-spin"
        ...
        >
          {busy === 'apple' ? (
            <span
              className="border-2 border-slate-600 border-t-white rounded-full animate-spin"
              style={{ width: 18, height: 18, display: 'inline-block' }}
            />
          ) : (
            <AppleLogo />
