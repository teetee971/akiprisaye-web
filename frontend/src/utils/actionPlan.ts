...
export function calculateBestTwoStores(products: Product[]): StoreOption {
  ...
  const sortedStores = Array.from(storeGroups.entries()).sort(
...
export function getBestOptions(products: Product[]): {
  ...
  if (savingsDiff > 2) {
    // Plus de 2€ de différence
    recommendation = 'B';
  } else if (savingsDiff < 0.5) {
    // Moins de 50 centimes de différence
    recommendation = 'A';
  }
 ...
 */
export function generateShoppingListText(products: Product[], option: StoreOption): string {
