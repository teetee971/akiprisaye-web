...
export function getAntiCrisisReading(
  ...
  const qualifiesForReading = isAntiCrise(ilpp, nationalAvg, stableCategories);
...
export function explainAntiCrisisReading(reading: AntiCrisisReadingResult): string {
  ...
  return (
    `${reading.territoryName} présente une pression de prix inférieure de ${diffText} points ` +
...
export function getAntiCrisisReadingStats(): {
  ...
  const territories = getAllTerritories();
  const allReadings = Object.keys(territories)
    .map((id) => getAntiCrisisReading(id))
    .filter((r) => r !== null) as AntiCrisisReadingResult[];
  ...
  return {
    ...
    percentageQualifying:
      allReadings.length > 0 ? (qualifying.length / allReadings.length) * 100 : 0,
