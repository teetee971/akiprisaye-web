...
export async function testObservatoireDataLoader() {
  ...
  try {
    ...
    console.log(`   - Snapshot 1: ${snapshots[0].date_snapshot} (${snapshots[0].donnees.length} observations)`);
    if (snapshots.length > 1) {
      console.log(`   - Snapshot 2: ${snapshots[1].date_snapshot} (${snapshots[1].donnees.length} observations)`);
    }
    ...
    stats.forEach(stat => {
      console.log(`   - ${stat.productName}: ${stat.avgPrice}€ (min: ${stat.minPrice}€, max: ${stat.maxPrice}€)`);
    });
...
export async function getObservatoireSummary(territory: string = 'Guadeloupe') {
  ...
  const avgDispersion = Array.from(dispersion.values())
    .reduce((sum, d) => sum + d.variance, 0) / dispersion.size;
