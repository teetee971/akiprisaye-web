/**
 * Tests pour le fournisseur Intermarché (Intermarché).
 */
import { describe, it, expect, vi, beforeEach } from 'vitest';
import { intermarcheProvider } from '../intermarcheProvider';

const INTERMARCHE_FLAG = 'VITE_PRICE_PROVIDER_INTERMARCHE';

const makeController = () => new AbortController();

beforeEach(() => {
  vi.restoreAllMocks();
  vi.stubEnv(INTERMARCHE_FLAG, 'false');
  vi.stubEnv('VITE_PRICE_API_BASE', '');
});

describe('intermarcheProvider', () => {
  it('has the correct source ID', () => {
    expect(intermarcheProvider.source).toBe('intermarche');
  });

  it('is disabled by default', () => {
    expect(intermarcheProvider.isEnabled()).toBe(false);
  });

  it('is enabled when env flag is true', () => {
    vi.stubEnv(INTERMARCHE_FLAG, 'true');
    expect(intermarcheProvider.isEnabled()).toBe(true);
  });

  it('returns NO_DATA when no barcode or query provided', async () => {
    vi.stubEnv(INTERMARCHE_FLAG, 'true');
    const result = await intermarcheProvider.search({}, makeController().signal);
  ...
  it('returns UNAVAILABLE when fetch throws', async () => {
    ...
    const result = await intermarcheProvider.search({ query: 'lait' }, makeController().signal);
  ...
  it('maps valid observations from API response', async () => {
    ...
    const result = await intermarcheProvider.search(
    ...
    expect(result.observations[0].source).toBe('intermarche');
  ...
  it('returns NO_DATA when API returns empty observations', async () => {
    ...
    const result = await intermarcheProvider.search({ query: 'xyz' }, makeController().signal);
    expect(result.status).toBe('NO_DATA');
    expect(result.observations).toEqual([]);
  ...
  it('filters out observations with invalid prices', async () => {
    ...
    const result = await intermarcheProvider.search({ query: 'produit' }, makeController().signal);
    expect(result.observations).toHaveLength(1);
  ...
  it('passes territory to request params', async () => {
    ...
    await intermarcheProvider.search({ query: 'riz', territory: 'gp' }, makeController().signal);
