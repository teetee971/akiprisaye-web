...
type MacaveCatalogResponse = {
  status?: string;
  observations?: MacaveCatalogItem[];
};
...
export const macaveLeclercProvider: PriceProvider = {
  ...
  async search(input: PriceSearchInput, signal: AbortSignal): Promise<ProviderResult> {
    ...
    const url = `${base}/api/macave-leclerc?${params.toString()}`;
    ...
    try {
      ...
      if (!response.ok) {
        return {
          ...
          status: 'UNAVAILABLE',
          observations: [],
          warnings: ['MaCave E.Leclerc temporairement indisponible.'],
        };
      }

      const payload = (await response.json()) as MacaveCatalogResponse;
      ...
      if (payload.status === 'UNAVAILABLE') {
        return {
          ...
          status: 'UNAVAILABLE',
          observations: [],
          warnings: ['MaCave E.Leclerc indisponible (amont).'],
        };
      }

      const items: MacaveCatalogItem[] = Array.isArray(payload.observations)
      ...
      return {
        ...
        warnings:
          observations.length === 0
            ? ['Aucun prix vins/champagnes E.Leclerc trouvé pour cette recherche.']
            : [],
      };
    } catch {
      return {
        ...
        status: 'UNAVAILABLE',
        observations: [],
        warnings: ['MaCave E.Leclerc indisponible.'],
      };
    }
  },
};
