...
describe('Actualites page', () => {
  ...
  it('builds /api/news query params from filters', async () => {
    ...
    await act(async () => {
      ...
      buttons.find(b => b.textContent === 'Bons plans')
        ?.dispatchEvent(new MouseEvent('click', { bubbles: true }));

      buttons.find(b => b.textContent === 'Fort')
        ?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
  ...
  it('shows fallback when fetch fails', async () => {
    ...
    expect(text).toMatch(/Actualités/i);
    expect(text).toMatch(/bons plans/i);
    expect(text).toMatch(/aucune donnée live disponible/i);
    expect(text).toMatch(/aucun résultat/i);
  });
});
