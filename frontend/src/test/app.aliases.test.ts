...
describe('App legacy alias routes', () => {
  it('keeps panier/cart/checkout aliases to /liste', () => {
    ...
    expect(appSource).toContain('<Route path="panier" element={<Navigate to="/liste" replace />} />');
    expect(appSource).toContain('<Route path="cart" element={<Navigate to="/liste" replace />} />');
    expect(appSource).toContain('<Route path="checkout" element={<Navigate to="/liste" replace />} />');
  });
});
