...
describe('deduplicateObservations', () => {
  it('keeps only one row per (retailer, territory)', () => {
    const obs = [
      makeObs({
        ...
        territory: 'GP',
        price: 3.5,
        observedAt: '2026-03-19T08:00:00Z',
      }),
      makeObs({
        retailer: 'Carrefour',
        territory: 'GP',
    ...
    expect(result[0].price).toBe(3.2);
  });
  ...
  it('treats name variants as the same retailer', () => {
    const obs = [
      makeObs({
        ...
        territory: 'MQ',
        price: 2.9,
        observedAt: '2026-03-19T10:00:00Z',
      }),
      makeObs({
        retailer: 'E.Leclerc',
        territory: 'MQ',
    ...
    expect(result[0].price).toBe(2.8);
  });

  it('keeps separate rows for different territories', () => {
    const obs = [
      makeObs({ retailer: 'Carrefour', territory: 'GP', price: 3.2 }),
  ...
  it('normalizes retailer name in output', () => {
    const obs = [makeObs({ retailer: 'super u', price: 3.6 })];
...
describe('discardOutliers', () => {
  ...
  it('returns input unchanged when fewer than 4 observations', () => {
    const obs = [makeObs({ price: 1.0 }), makeObs({ price: 5.0 }), makeObs({ price: 100.0 })];
