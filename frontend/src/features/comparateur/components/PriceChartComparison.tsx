...
export function PriceChart({ data, type = 'bar' }: PriceChartProps) {
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 space-y-4">
      <div>
        <h3 className="text-xl font-semibold text-white mb-2">Visualisation des prix par territoire</h3>
        <p className="text-sm text-slate-400">Comparaison visuelle des prix - du moins cher au plus cher</p>
      </div>
      ...
      <ResponsiveContainer width="100%" height={400}>
        ...
        >
          ...
          <Tooltip 
            ...
            formatter={(value) => [`${(typeof value === 'number' ? value : 0).toFixed(2)}€`, 'Prix']}
          ...
          >
            {chartData.map((entry, index) => (
              <Cell 
                key={`cell-${index}`}
                fill={entry.prix === minPrice ? '#4ade80' : '#60a5fa'}
              />
            ))}
          </Bar>
        </BarChart>
