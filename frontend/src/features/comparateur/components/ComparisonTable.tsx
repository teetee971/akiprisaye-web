    ...
    return (
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6">
        <p className="text-center text-slate-400">
          Aucune donnée de prix disponible pour ce produit
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
                    ...
                    >
                      <td className="py-3 px-4">
                        <span className="text-white font-medium">
                          {getTerritoryLabel(tp.territory)}
                        </span>
                      </td>
                      ...
                      <td
                        className={`py-3 px-4 text-right ${isHighDiff ? 'text-red-400 font-semibold' : 'text-slate-300'}`}
                      >
                      ...
                      >
                        <div className="flex items-center justify-end gap-2">
                          {diff === 0 ? '—' : `+${diffPercent}%`}
                          {isHighDiff && <span title="Écart important (>20%)">⚠️</span>}
                        </div>
                      </td>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Prix minimum" value={`${minPrice.toFixed(2)}€`} icon="⬇️" />
        <StatCard label="Prix maximum" value={`${maxPrice.toFixed(2)}€`} icon="⬆️" />
        <StatCard label="Prix moyen" value={`${avgPrice.toFixed(2)}€`} icon="📊" />
        <StatCard label="Écart max" value={`${maxDiffPercent}%`} icon="📏" />
      </div>
    </div>
  );
}
