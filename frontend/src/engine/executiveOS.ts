 ...
 *     stability × 0.10
 */
export function computePlatformHealth(signals: PlatformSignals): ExecutiveSnapshot['dimensions'] & { overall: number } {
  ...
  return { traffic: Math.round(traffic), revenue: Math.round(revenue), retention: Math.round(retention), data: Math.round(data), stability: Math.round(stability), overall: Math.min(100, overall) };
}
...
export function buildExecutiveSnapshot(signals: PlatformSignals): ExecutiveSnapshot {
  ...
  return { healthScore: overall, healthStatus: status, kpis, risks, decisions, dimensions, generatedAt: Date.now() };
}
