...
export default function ListePage() {
      ...
      simulateMonthlySavings(
        items.map((item) => ({
          ...
          priceHistory: (item.history ?? []).map((price, index, array) => ({
            price,
            observedAt: new Date(
              Date.now() - (array.length - 1 - index) * 24 * 60 * 60 * 1000
            ).toISOString(),
          })),
        }))
      ),
  ...
  const displaySavings = isPremium
    ? `${savingsSimulation.potentialSavings.toFixed(2)} €`
    : `${Math.floor(savingsSimulation.potentialSavings)}€ ···`;
  ...
  const exportData = () => {
    const payload = can('EXPORT_ADVANCED')
      ...
      : JSON.stringify(
          items.map(({ id, name: itemName, quantity }) => ({ id, name: itemName, quantity }))
        );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <input
          className="flex-1 min-w-0 rounded border border-slate-700 bg-slate-900 px-3 py-2"
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="Ajouter un article"
        />
        <button className="rounded bg-blue-600 px-4 py-2 whitespace-nowrap" onClick={onAdd}>
          Ajouter
        </button>
        <button
          className="rounded bg-slate-700 px-4 py-2 whitespace-nowrap"
          onClick={refreshPrices}
        >
          Refresh prix
        </button>
        <button className="rounded bg-emerald-700 px-4 py-2 whitespace-nowrap" onClick={exportData}>
          Exporter
        </button>
      </div>

      {isAtMax && <UpgradeBanner usedItems={items.length} className="mb-2" />}

      <div className="rounded-lg border border-slate-700 p-3">
        <h2 className="font-semibold">Territoires</h2>
        <p className="text-sm text-slate-300">
          {can('MULTI_TERRITORY') ? 'Multi-territoires activé' : explain('MULTI_TERRITORY')}
        </p>
      ...
      <div className="rounded-lg border border-slate-700 p-3">
        ...
        {can('PRICE_HISTORY_ADVANCED') && stats ? (
          <p className="text-sm text-slate-200">
            Min: {stats.min.toFixed(2)} · Max: {stats.max.toFixed(2)} · Moy: {stats.avg.toFixed(2)}{' '}
            · P90: {stats.p90.toFixed(2)} · Volatilité: {stats.vol.toFixed(2)}
          </p>
        ) : (
          <p className="text-sm text-slate-300">
            Historique simple disponible (dernière observation affichée).
          </p>
        )}
      </div>
        ...
        {can('PRICE_ALERTS') ? (
          <div className="flex gap-2 text-sm">
            <input
              ...
              onChange={(e) => setAlerts((a) => ({ ...a, threshold: Number(e.target.value) }))}
            />
            <input
              className="rounded border border-slate-700 bg-slate-900 px-2 py-1"
              type="number"
              placeholder="Baisse %"
              onChange={(e) => setAlerts((a) => ({ ...a, dropPercent: Number(e.target.value) }))}
            />
      ...
      <div className="rounded-lg border border-slate-700 p-3">
        ...
        <div className="mt-3 rounded border border-indigo-500/40 bg-indigo-950/20 p-3 text-sm">
          ...
          <p className="text-indigo-100">
            Vous auriez pu économiser ce mois-ci : <strong>{displaySavings}</strong>
          </p>
          {!isPremium && (
            <button
              className="mt-2 rounded bg-indigo-600 px-3 py-1 text-xs"
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs">
          {[
            'Voir tendance 30 jours',
            'Voir recommandation détaillée',
            'Activer alerte personnalisée',
          ].map((label) => (
            <button
              key={label}
              type="button"
      ...
      <ul className="space-y-2">
        {items.map((item, index) => {
          ...
          const previousAisle =
            index > 0 ? getAisleFromCategory((items[index - 1] as any).category) : null;
              ...
              <li className="rounded border border-slate-700 p-3">
                <div className="flex items-start gap-3">
                  {item.imageThumbUrl ? (
                    ...
                    <div
                      data-testid="item-thumb-placeholder"
                      className="h-14 w-14 rounded-md border border-slate-700 bg-slate-800/80"
                      aria-hidden="true"
                    />
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() => setItems(removeShoppingListItem(item.id))}
                      >
                        Retirer
                      </button>
                    </div>
                    <div className="mt-1 text-xs text-slate-400">
                      Dernier prix: {lastPrice ? `${lastPrice.toFixed(2)} €` : 'N/A'} · Source:{' '}
                      {item.source ?? 'local'} · Date:{' '}
                      {item.lastObservedAt ? new Date(item.lastObservedAt).toLocaleString() : 'N/A'}
                    </div>
                    {isPremiumEnabled && isPremium && (
                      <div className="mt-1 rounded border border-indigo-500/40 bg-indigo-900/20 px-2 py-1 text-xs text-indigo-100">
                        <span>
                          {item.normalized?.normalizedLabel ?? 'Prix unitaire indisponible'} · Trend
                          7j: {item.premium?.trend7 ?? 'flat'} · Trend 30j:{' '}
                          {item.premium?.trend30 ?? 'flat'}
                        </span>
                        <span className="ml-2">Score: {item.premium?.score ?? 0}/100</span>
                        {item.premium?.alerts?.length ? (
                          <span className="ml-2">Alertes: {item.premium.alerts.join(' | ')}</span>
                        ) : null}
                      </div>
                    )}
                    {rec && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-lg rounded-xl border border-indigo-500/40 bg-slate-900 p-5">
            ...
            <p className="mt-2 text-sm text-slate-300">
              Détection automatique des meilleures périodes, alertes personnalisées, comparaison
              multi-magasins et simulation d'économies mensuelles.
            </p>
            <div className="mt-4 rounded border border-indigo-500/30 bg-indigo-950/20 p-3 text-sm">
              <p>
                Premium: <strong>2.99€ / mois</strong> ou <strong>24.99€ / an</strong>
              </p>
            </div>
