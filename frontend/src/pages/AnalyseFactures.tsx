...
import {
  Upload, FileText, BarChart2, Download, Trash2, ChevronLeft, ChevronRight,
} from 'lucide-react';
...
const CATEGORIES: Category[] = [
  'Alimentation', 'Électricité', 'Eau', 'Télécom', 'Carburant', 'Santé', 'Divers',
];
...
function generateMockItems(filename: string): LineItem[] {
  ...
  const items: LineItem[] = [
    { description: 'Courses supermarché', category: 'Alimentation', amount: 87.50 + seed * 5 },
    { description: 'Électricité EDF', category: 'Électricité', amount: 42.30 + seed * 2 },
    { description: 'Facture eau', category: 'Eau', amount: 18.75 },
    { description: 'Abonnement mobile', category: 'Télécom', amount: 19.99 },
    { description: 'Carburant SP95', category: 'Carburant', amount: 65.00 + seed * 3 },
    { description: 'Pharmacie', category: 'Santé', amount: 23.60 },
    { description: 'Divers achats', category: 'Divers', amount: 31.20 + seed * 1.5 },
  ];
  return items;
}
...
export default function AnalyseFactures() {
  ...
  const allItems = useMemo(
    () => visibleInvoices.flatMap((i) => i.items),
    [visibleInvoices],
  );
  ...
  function handleExportAll() {
    if (allItems.length === 0) { toast.error('Aucune donnée à exporter'); return; }
      ...
      <div className='min-h-screen bg-gray-50'>
        ...
        <div className='max-w-3xl mx-auto px-4 py-6 pb-12 space-y-6'>
          ...
          >
            ...
            />
            <Upload className={`w-10 h-10 mx-auto mb-3 ${dragging ? 'text-violet-600' : 'text-gray-300'}`} />
            {parsing ? (
                    ...
                    <div className='flex items-start justify-between gap-2 mb-3'>
                      <div>
                        ...
                        <p className='text-xs text-gray-400 mt-0.5'>
                          {new Date(inv.parsedAt).toLocaleDateString('fr-FR')} · {inv.total.toFixed(2)} €
                        </p>
                      </div>
                      <div className='flex items-center gap-2'>
                        <button
                    ...
                    <div className='space-y-1'>
                      {inv.items.map((item, i) => (
                        <div key={i} className='flex items-center justify-between text-xs'>
                          ...
                          <span className={`ml-2 px-1.5 py-0.5 rounded text-white text-[10px] flex-shrink-0 ${CATEGORY_COLORS[item.category]}`}>
                            {item.category}
                          </span>
