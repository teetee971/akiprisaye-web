...
import { RefreshCw, Wifi, WifiOff, Clock, AlertCircle, ToggleLeft, ToggleRight } from 'lucide-react';
  ...
  return (
    <div className='max-w-6xl mx-auto px-4 py-8 space-y-8'>
      ...
      <div className='flex items-center justify-between flex-wrap gap-4'>
        ...
        >
          {simMode ? <ToggleRight className='text-blue-500 w-5 h-5' /> : <ToggleLeft className='text-gray-400 w-5 h-5' />}
          Mode simulation
        </button>
      </div>
          ...
          >
            <div className='flex flex-wrap items-start justify-between gap-4'>
              ...
              <div className='flex items-center gap-3'>
                ...
                >
                  <RefreshCw className={`w-4 h-4 ${syncing.has(partner.id) ? 'animate-spin' : ''}`} />
            ...
            <div className='mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm'>
              ...
              <div className='bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2'>
                ...
                <p className={`font-medium ${partner.latencyMs > 300 ? 'text-orange-600' : 'text-green-600'}`}>
                  {partner.latencyMs > 0 ? `${partner.latencyMs} ms` : '—'}
                </p>
              </div>
              <div className='bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2'>
                <p className='text-xs text-gray-500 dark:text-gray-400'>Uptime</p>
                <p className={`font-medium ${partner.uptimePct > 99 ? 'text-green-600' : partner.uptimePct > 95 ? 'text-orange-600' : 'text-red-600'}`}>
                  {partner.uptimePct}%
                </p>
              </div>
            </div>
                ...
                {expandedErrors.has(partner.id) && (
                  <ul className='mt-2 space-y-1'>
                    {partner.errors.map((e, i) => (
                      <li key={i} className='text-xs text-gray-600 dark:text-gray-400 font-mono bg-orange-50 dark:bg-orange-900/10 rounded px-2 py-1'>
                        {e}
                      </li>
                    ))}
                  </ul>
