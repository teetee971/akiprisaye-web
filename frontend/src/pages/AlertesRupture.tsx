...
const CATEGORIE_PRODUITS = [
  'Riz, pâtes, farine', 'Huile, beurre', 'Lait, yaourts, fromages',
  'Viandes, poissons', 'Fruits et légumes', 'Conserves', 'Eau minérale',
  'Hygiène, cosmétiques', 'Couches, articles bébé', 'Médicaments sans ordonnance',
  'Carburant', 'Bouteilles de gaz', 'Appareils électriques', 'Autre',
];

// ─── Mock data (shown if Firestore is unavailable) ────────────────────────────

const MOCK_RUPTURES: Rupture[] = [
  { produit: 'Huile de tournesol 1L', magasin: 'Carrefour', territoire: 'gp', commentaire: 'Rayon vide depuis 3 jours', createdAt: '2026-03-06', statut: 'ouvert' },
  { produit: 'Couches bébé taille 4', magasin: 'Leader Price', territoire: 'mq', commentaire: 'Plus de stock, pas de réassort annoncé', createdAt: '2026-03-05', statut: 'ouvert' },
  { produit: 'Lait UHT demi-écrémé', magasin: 'Champion', territoire: 're', commentaire: '', createdAt: '2026-03-04', statut: 'resolu' },
  { produit: 'Eau minérale 6x1,5L', magasin: 'Hyper U', territoire: 'gf', commentaire: 'Rupture pendant 5 jours', createdAt: '2026-03-01', statut: 'resolu' },
];
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: '1.5rem 1rem 3rem' }}>
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', padding: '0.3rem 1rem', borderRadius: 20, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', marginBottom: '0.75rem' }}>
            <span style={{ fontSize: '0.78rem', color: '#f87171', fontWeight: 700 }}>🚨 Signalement citoyen</span>
          </div>
          <h1 style={{ margin: '0 0 0.4rem', fontSize: '1.7rem', fontWeight: 900, color: '#f1f5f9' }}>
            Alertes ruptures de stock
          </h1>
          <p style={{ margin: 0, fontSize: '0.88rem', color: '#64748b' }}>
            Signalez les ruptures de produits de première nécessité dans votre territoire
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          {[
            { label: 'Signalements actifs', value: ouvert, color: '#ef4444', bg: 'rgba(239,68,68,0.08)', border: 'rgba(239,68,68,0.25)' },
            { label: 'Résolus récemment', value: resolu, color: '#22c55e', bg: 'rgba(34,197,94,0.08)', border: 'rgba(34,197,94,0.25)' },
            { label: 'Total signalements', value: ruptures.length, color: '#6366f1', bg: 'rgba(99,102,241,0.08)', border: 'rgba(99,102,241,0.25)' },
          ].map(s => (
            <div key={s.label} style={{ flex: '1 1 150px', padding: '0.8rem 1rem', borderRadius: 12, background: s.bg, border: `1px solid ${s.border}`, textAlign: 'center' }}>
              <div style={{ fontSize: '1.6rem', fontWeight: 900, color: s.color, lineHeight: 1 }}>{s.value}</div>
              <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginTop: '0.15rem' }}>{s.label}</div>
            </div>
          ))}
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} style={{ padding: '1.25rem 1.4rem', borderRadius: 16, background: 'rgba(15,23,42,0.75)', border: '1px solid rgba(148,163,184,0.12)', marginBottom: '1.5rem' }}>
          <h2 style={{ margin: '0 0 1rem', fontSize: '1rem', fontWeight: 800, color: '#e2e8f0' }}>
            ➕ Signaler une rupture
          </h2>

          {success && (
            <div style={{ padding: '0.7rem 1rem', borderRadius: 8, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.35)', color: '#86efac', fontSize: '0.82rem', marginBottom: '0.85rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '0.75rem' }}>
            <div>
              ...
              <datalist id="produits-list">
                {CATEGORIE_PRODUITS.map(p => <option key={p} value={p} />)}
              </datalist>
            </div>
            <div>
          ...
          <button type="submit" disabled={submitting || !form.produit || !form.magasin}
            style={{ padding: '0.55rem 1.5rem', borderRadius: 8, background: submitting ? 'rgba(99,102,241,0.4)' : 'rgba(99,102,241,0.85)', color: '#fff', fontSize: '0.85rem', fontWeight: 700, border: 'none', cursor: submitting ? 'not-allowed' : 'pointer' }}>
            {submitting ? '⏳ Envoi…' : '🚨 Signaler cette rupture'}
          </button>

          <p style={{ margin: '0.5rem 0 0', fontSize: '0.65rem', color: '#334155' }}>
            Votre signalement est anonyme et public. Les données sont partagées avec les OPMR locaux.
          </p>
        </form>

        {/* List */}
        <div>
          <h2 style={{ margin: '0 0 0.85rem', fontSize: '1rem', fontWeight: 800, color: '#e2e8f0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            📋 Signalements récents
            {!db && <span style={{ fontSize: '0.65rem', color: '#475569', fontWeight: 400 }}>(données exemples — Firestore non configuré)</span>}
          </h2>
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {ruptures.map((r, i) => (
                <div key={r.id || i} style={{ padding: '0.85rem 1rem', borderRadius: 12, background: 'rgba(15,23,42,0.65)', border: `1px solid ${r.statut === 'resolu' ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)'}`, display: 'flex', alignItems: 'flex-start', gap: '0.75rem', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 180 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.2rem' }}>
                      <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#e2e8f0' }}>{r.produit}</span>
                      <span style={{ fontSize: '0.65rem', padding: '1px 7px', borderRadius: 20, fontWeight: 700,
                        background: r.statut === 'resolu' ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)',
                    ...
                    {r.commentaire && <p style={{ margin: '0.25rem 0 0', fontSize: '0.72rem', color: '#94a3b8', lineHeight: 1.5 }}>{r.commentaire}</p>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ marginTop: '1.5rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap', justifyContent: 'center' }}>
          <Link to="/comparateur" style={{ padding: '0.55rem 1.2rem', borderRadius: 8, background: 'rgba(99,102,241,0.75)', color: '#fff', fontSize: '0.82rem', fontWeight: 700, textDecoration: 'none' }}>
            🔍 Comparer les prix
          </Link>
          <a href="https://www.guadeloupe.gouv.fr/Politiques-publiques/Economie-emploi-et-entreprises/Observatoire-des-Prix-des-Marges-et-des-Revenus" target="_blank" rel="noopener noreferrer"
            style={{ padding: '0.55rem 1.2rem', borderRadius: 8, background: 'rgba(30,41,59,0.8)', border: '1px solid rgba(148,163,184,0.2)', color: '#94a3b8', fontSize: '0.82rem', fontWeight: 600, textDecoration: 'none' }}>
            🏛️ Contacter l'OPMR
          </a>
        </div>
