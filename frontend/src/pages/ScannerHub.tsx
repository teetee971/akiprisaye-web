...
function StatusBadge({ status }: { status: ScanStatus }) {
  ...
  const label =
    status === 'loading' ? '…' : status === 'ok' ? 'OK' : status === 'not_found' ? 'VIDE' : 'ERR';
  return (
    <span
      className={`inline-flex h-6 min-w-11 items-center justify-center rounded-full border px-2 text-xs font-semibold ${tone}`}
    >
      {label}
    </span>
  );
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Scanner continu: code-barres avec historique et ajout rapide au panier"
        />{' '}
        <link rel="canonical" href="https://teetee971.github.io/akiprisaye-web/scanner" />
        <link
          rel="alternate"
          hrefLang="fr"
          href="https://teetee971.github.io/akiprisaye-web/scanner"
        />
        <link
          rel="alternate"
          hrefLang="x-default"
      ...
      <div className="min-h-screen bg-slate-950 p-4 pt-14 text-white">
        <section className="mx-auto w-full max-w-4xl rounded-2xl border border-slate-800 bg-slate-900 p-4">
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            ...
            {listCount > 0 && (
              <button
                ...
                className="rounded-lg border border-blue-400/60 px-4 py-3 text-sm font-semibold text-blue-100"
              >
                Voir la liste ({listCount})
              </button>
            )}
            <button
              type="button"
          ...
          <div className="mt-3 text-sm text-slate-300">
            {!barcodeSupport && (
              <div className="space-y-3 rounded-lg border border-blue-500/30 bg-blue-500/10 p-3">
                ...
                <button
                  ...
                  className="rounded-lg border border-blue-500/60 px-3 py-2 text-xs font-semibold text-blue-200"
                >
                  Utiliser le scan manuel (/scan-ean)
                </button>
                <div className="flex flex-wrap gap-2">
                  <input
                    value={manualEan}
                    onChange={(e) => setManualEan(e.target.value)}
                    placeholder="Saisir EAN (fallback)"
                    className="rounded border border-slate-600 bg-slate-900 px-2 py-1 text-sm"
                    aria-label="Saisir un code EAN manuellement"
                  />
                  <button
                    type="button"
                    onClick={handleManualLookup}
                    className="rounded border border-blue-500/60 px-3 py-1 text-xs font-semibold text-blue-100"
                  >
                    Rechercher
                  </button>
                </div>
                ...
                {manualResult ? (
                  <div className="text-xs text-emerald-300">
                    ✅ {manualResult.name}
                    {manualResult.brand ? ` — ${manualResult.brand}` : ''}
                  </div>
                ) : null}
              </div>
            )}
            ...
            {!cameraError &&
              barcodeSupport &&
              'Astuce: gardez le code au centre 1-2 secondes pour stabiliser la détection.'}
          </div>

          {debugEnabled && (
            <div className="mt-4 rounded-xl border border-amber-500/50 bg-amber-500/10 p-3 text-sm text-amber-100">
              <p>
                <strong>Mode debug</strong> (debug=1)
              </p>
            ...
            {results.length === 0 ? (
              ...
              <div className="grid gap-3">
                {results.map((item) => (
                  <div
                    key={item.id}
                    className="flex flex-col justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4"
                  >
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <span className="font-semibold text-white">
                                {item.product.name || `Produit (EAN: ${item.barcode})`}
                              </span>
                              {item.product.brand ? <span> — {item.product.brand}</span> : null}
                              {item.product.quantity ? (
                                <div className="text-xs text-gray-400">{item.product.quantity}</div>
                              ) : null}
                              <div className="mt-1 flex flex-wrap gap-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() => navigate(`/produit/${item.barcode}`)}
                    ...
                    <div className="flex shrink-0 gap-2">
                      <button
                        type="button"
                        onClick={() => handleAddManual(item.id)}
