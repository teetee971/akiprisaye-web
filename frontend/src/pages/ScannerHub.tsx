...
export default function ScannerHub() {
  ...
  return (
    <>
      <Helmet><title>Scanner - A KI PRI SA YÉ</title><meta name="description" content="Scanner continu: code-barres avec historique et ajout rapide au panier" />        <link rel="canonical" href="https://teetee971.github.io/akiprisaye-web/scanner" />
                  ...
                  <div key={item.id} className="flex flex-col justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 text-sm text-gray-300">
                        ...
                        {item.status === 'ok' && item.product && <div className="flex items-start gap-3">{item.product.imageUrl ? <img src={item.product.imageUrl} alt={item.product.name} width={56} height={56} className="h-14 w-14 rounded object-cover" loading="lazy" /> : null}<div><span className="font-semibold text-white">{item.product.name || `Produit (EAN: ${item.barcode})`}</span>{item.product.brand ? <span> — {item.product.brand}</span> : null}{item.product.quantity ? <div className="text-xs text-gray-400">{item.product.quantity}</div> : null}<div className="mt-1 flex flex-wrap gap-2"><button type="button" onClick={() => navigate(`/product/${item.barcode}`)} className="text-xs text-blue-300 underline">Voir fiche</button><button type="button" onClick={() => navigate(`/produit/${item.barcode}`)} className="text-xs text-emerald-300 underline">🏷️ Comparer les prix</button></div></div></div>}
                      </div>
                    </div>
