...
export default function AdminTicketImport() {
  ...
  const handlePhotoUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (e: any) { setErrorMessage(e.message); } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'receipts', id), { ...parsedPayload, created_at: serverTimestamp() });
      setStatusMessage('Ticket enregistré !');
    } catch (e: any) { setErrorMessage(e.message); } finally { setLoading(false); }
  };

  return (
    <div className="max-w-5xl mx-auto p-6 text-white space-y-6 bg-slate-900 min-h-screen">
      <Helmet><title>Admin - Ticket</title></Helmet>
      <h1 className="text-2xl font-black flex items-center gap-2"><ReceiptText /> Import Tickets</h1>
      ...
      <textarea value={jsonInput} onChange={(e) => setJsonInput(e.target.value)} className="w-full h-80 p-4 bg-slate-950 rounded-xl font-mono text-sm" />
      <div className="flex gap-4">
        <button onClick={handleAnalyze} className="px-6 py-2 bg-blue-500 text-slate-950 font-bold rounded-lg">Analyser le ticket</button>
        <button onClick={handleSave} disabled={!parsedPayload || loading} className="px-6 py-2 bg-emerald-500 text-slate-950 font-bold rounded-lg disabled:opacity-50">
          {loading ? <Loader2 className="animate-spin" /> : 'Enregistrer en base de données'}
        </button>
      </div>
      {errorMessage && <div className="text-red-400">{errorMessage}</div>}
      {statusMessage && <div className="text-emerald-400">{statusMessage}</div>}
    </div>
  );
}
