...
export function ImportPage() {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === 'stores') {
        ...
        result = await importProductsFromCSV(content, (current, total) => {
          setImportProgress(Math.round((current / total) * 100));
        });
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      {/* Header */}
      <div className="mb-8">
        <h1 className="mb-2 text-3xl font-bold text-slate-900">Import CSV</h1>
      ...
      <div className="space-y-6">
          ...
          <GlassCard>
            <div className="flex items-start space-x-3 mb-4">
              ...
              <div className="flex-1">
                <h3 className="mb-2 text-lg font-semibold text-slate-900">Instructions d'import</h3>
                <ul className="space-y-1 text-sm text-slate-700">
                  {currentTab.instructions.map((instruction, index) => (
                    <li
                      key={instruction}
                      className={index === 0 ? 'font-medium text-slate-800' : ''}
                    >
                      {instruction}
                    </li>
                  ))}
        ...
        {step === 'upload' && (
          <GlassCard>
            <h3 className="mb-4 text-lg font-semibold text-slate-900">Sélectionner un fichier</h3>
            <CsvUploader
        ...
        {step === 'preview' && (
          <>
            <GlassCard>
              <h3 className="mb-4 text-lg font-semibold text-slate-900">Aperçu des données</h3>
              <ImportPreview
                data={csvData}
        ...
        {step === 'importing' && (
          <GlassCard>
            <div className="text-center py-8">
              ...
              <h3 className="mb-2 text-xl font-semibold text-slate-900">Import en cours...</h3>
              <p className="mb-4 text-slate-600">Veuillez patienter pendant l'import des données</p>
