...
const CarRentalComparator: React.FC = () => {
  ...
  useEffect(() => {
    if (!filteredPrices.length) { setComparisonResult(null); return; }
  ...
  const chartData = useMemo(() => {
    ...
    const sorted = [...comparisonResult.agencies].sort((a, b) => a.rentalPrice.pricing.dailyRate - b.rentalPrice.pricing.dailyRate);
    return {
      labels: sorted.map((a) => a.rentalPrice.agency),
      datasets: [{ label: 'Prix/jour (€)', data: sorted.map((a) => a.rentalPrice.pricing.dailyRate), backgroundColor: 'rgba(16,185,129,0.6)' }],
    };
  }, [comparisonResult]);
  ...
  const exportCSV = () => {
    if (!comparisonResult) return;
    const rows = [
      ['Agence', 'Catégorie', 'Prix/jour', 'Dépôt', 'Km illimités', 'Agence locale', 'Date observation'],
    ...
    a.href = url; a.download = `location-voiture-${selectedTerritory}-${selectedCategory}.csv`; a.click();
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-4 pb-12 pt-6">
        ...
        <div className="flex items-start gap-2 bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 mb-6 text-sm text-blue-300">
          ...
          <p>
            <strong>Observer, pas vendre.</strong> Ces prix sont indicatifs et basés sur des relevés citoyens et les tarifs publiés par les agences. 
            Vérifiez toujours le prix final sur le site de l'agence. Aucun lien d'affiliation.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="flex items-end">
            <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-300">
              <input type="checkbox" checked={localOnly} onChange={(e) => setLocalOnly(e.target.checked)} className="w-4 h-4 accent-emerald-500" />
          ...
          <>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 my-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => setSortDirection((d) => d === 'asc' ? 'desc' : 'asc')} className="flex items-center gap-1 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm">
                ...
                <ShareButton title="Comparateur Location Voiture" description="Comparez les prix de location de voiture dans les territoires ultramarins" />
              </div>
            </div>

            <div className="bg-slate-900 rounded-xl p-4 my-4">
              <h2 className="text-sm font-semibold text-gray-300 mb-3 flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-emerald-400" />Prix/jour par agence (€)
              </h2>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {sortedAgencies.map((item) => (
                    <tr key={item.rentalPrice.id} className="border-t border-slate-800 hover:bg-slate-800/50">
                      <td className="px-4 py-3 text-white font-medium">{item.rentalPrice.agency}</td>
                      <td className="px-4 py-3 text-gray-400 text-xs">{item.rentalPrice.vehicleExample}</td>
                      <td className="px-4 py-3 text-right font-semibold text-white">{formatPrice(item.rentalPrice.pricing.dailyRate)}</td>
                      <td className="px-4 py-3 text-right text-gray-400">{formatPrice(item.rentalPrice.pricing.deposit)}</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        {item.rentalPrice.isLocalAgency ? (
                          <span className="text-xs bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded-full">Locale</span>
                        ) : (
                          <span className="text-xs bg-slate-700 text-gray-400 px-2 py-0.5 rounded-full">Inter.</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
