...
async function loadPrix(territory: string, months: string[]) {
  ...
  // Average by produit
  const byProduit = new Map<string, { sum: number; count: number; unite: string; categorie: string }>();
  for (const e of all) {
    const k = e.produit.toLowerCase();
    const cur = byProduit.get(k) ?? { sum: 0, count: 0, unite: e.unite, categorie: e.categorie };
    byProduit.set(k, { sum: cur.sum + e.prix, count: cur.count + 1, unite: e.unite, categorie: e.categorie });
  }
  const result = new Map<string, { avg: number; unite: string; categorie: string }>();
  for (const [k, v] of byProduit.entries()) {
    result.set(k, { avg: Math.round((v.sum / v.count) * 100) / 100, unite: v.unite, categorie: v.categorie });
  }
  return result;
}
...
function computePanier(panier: PanierType, domMap: Map<string, { avg: number; unite: string; categorie: string }>, hexMap: Map<string, { avg: number; unite: string; categorie: string }>): PanierResult {
...
export default function PaniersTypes() {
  ...
  const [domPrices, setDomPrices] = useState<Map<string, { avg: number; unite: string; categorie: string }>>(new Map());
  const [hexPrices, setHexPrices] = useState<Map<string, { avg: number; unite: string; categorie: string }>>(new Map());
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-3 items-center justify-between">
          ...
          <div>
            <label htmlFor="paniers-territoire-select" className="block text-xs text-slate-400 mb-1">Territoire</label>
            <select
              id="paniers-territoire-select"
          ...
          <div>
            <span className="block text-xs text-slate-400 mb-1">Panier</span>
        ...
        {!loading && hasData && (
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-5 space-y-4">
            <div className="flex items-start justify-between flex-wrap gap-4">
              <div>
                <h2 className="text-lg font-bold text-white">{panier.emoji} {panier.label}</h2>
                <p className="text-sm text-slate-400">{panier.description}</p>
              </div>
              {result.ecartPercent !== 0 && result.totalHex > 0 && (
                <EcartHexagone
                  ecartPercent={result.ecartPercent}
                  size="md"
                  showTooltip={false}
                />
              )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              {result.totalHex > 0 && (
                <div className="bg-slate-800 rounded-lg p-3 text-center">
                  <div className="text-2xl font-bold text-slate-300">{result.totalHex.toFixed(2)} €</div>
              ...
              {result.ecartTotal !== 0 && result.totalHex > 0 && (
                <div className={[
                  ...
                  <div className={[
                    ...
                    {result.ecartTotal > 0 ? '+' : ''}{result.ecartTotal.toFixed(2)} €
                  </div>
                      ...
                      {result.items.map((item, idx) => (
                        <tr key={idx} className="border-b border-slate-800/50">
                          ...
                          <td className="py-1.5 pr-4 text-right text-slate-200">
                            {item.prixDOM !== undefined ? `${item.prixDOM.toFixed(2)} €` : <span className="text-slate-600">—</span>}
                          </td>
                          <td className="py-1.5 pr-4 text-right text-slate-400">
                            {item.prixHex !== undefined ? `${item.prixHex.toFixed(2)} €` : <span className="text-slate-600">—</span>}
                          </td>
        ...
        {!loading && !hasData && (
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-6 text-center space-y-2">
            ...
            <p className="text-slate-400">
              Données non disponibles pour {selectedTerritory}.
            </p>
        ...
        <div className="flex gap-2 text-xs text-slate-500 bg-slate-900/50 border border-slate-800 rounded-lg p-3">
          ...
          <span>
            ...
            L'écart DOM / Métropole est indicatif — il peut varier selon l'enseigne, la commune et la saisonnalité.
          </span>
        </div>
      </div>
    </div>
