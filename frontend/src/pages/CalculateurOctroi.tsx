...
const CATEGORIES: Category[] = [
  ...
  { id: 'boissons',  label: 'Boissons non alcoolisées',    emoji: '🧃', octroi: 8,  margeDOM: 35, margeFR: 24 },
  { id: 'viande',    label: 'Viandes, charcuterie',        emoji: '🥩', octroi: 10, margeDOM: 38, margeFR: 28 },
  { id: 'hygiene',   label: 'Hygiène, cosmétiques',        emoji: '🧴', octroi: 15, margeDOM: 42, margeFR: 30 },
  ...
  { id: 'vetement',  label: 'Vêtements, textiles',         emoji: '👕', octroi: 18, margeDOM: 45, margeFR: 35 },
];
...
export default function CalculateurOctroi() {
  const [prixUsine, setPrixUsine] = useState(2.00);
  ...
  const steps = [
    ...
    },
    {
      ...
      add: montantMarge,
      color: '#f59e0b',
      source: { label: 'Autorité de la concurrence — Avis 09-A-45', url: 'https://www.autoritedelaconcurrence.fr/fr/decision/relatif-au-fonctionnement-de-la-grande-distribution-dans-les-departements-doutre-mer' },
      note: terr.code === 'fr' ? `Marge moyenne en métropole : ${pct(cat.margeFR)}.` : `Marge moyenne en DOM : ${pct(cat.margeDOM)} (vs ${pct(cat.margeFR)} en métropole). Oligopole de distribution.`,
    },
    {
      ...
      add: montantTVA,
      color: '#22c55e',
      source: { label: 'CGI art. 296 — TVA DOM 8,5 %', url: 'https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000006309725' },
      note: terr.code === 'fr' ? 'TVA normale 20 % en métropole.' : 'Taux réduit TVA DOM : 8,5 % (vs 20 % en métropole).',
    },
  ];
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: '1.5rem 1rem 3rem' }}>
      <div style={{ maxWidth: 780, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ padding: '1rem 1.1rem', borderRadius: 12, background: 'rgba(15,23,42,0.75)', border: '1px solid rgba(148,163,184,0.12)' }}>
            ...
            <input
              ...
              step={0.10}
              value={prixUsine}
              onChange={e => setPrixUsine(Math.max(0.10, parseFloat(e.target.value) || 0.10))}
              style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: 8, background: 'rgba(30,41,59,0.8)', border: '1px solid rgba(99,102,241,0.4)', color: '#f1f5f9', fontSize: '1.1rem', fontWeight: 700, boxSizing: 'border-box' }}
            />
            <p style={{ margin: '0.3rem 0 0', fontSize: '0.65rem', color: '#475569' }}>Prix de revient fabricant hors taxes</p>
          </div>
          ...
          <div style={{ padding: '1rem 1.1rem', borderRadius: 12, background: 'rgba(15,23,42,0.75)', border: '1px solid rgba(148,163,184,0.12)' }}>
            ...
            <p style={{ margin: '0.3rem 0 0', fontSize: '0.65rem', color: '#475569' }}>Détermine le taux d'octroi de mer</p>
          </div>
          ...
          <div style={{ padding: '1rem 1.1rem', borderRadius: 12, background: 'rgba(15,23,42,0.75)', border: '1px solid rgba(148,163,184,0.12)' }}>
            ...
            <p style={{ margin: '0.3rem 0 0', fontSize: '0.65rem', color: '#475569' }}>Détermine le fret et la TVA applicables</p>
          </div>
        </div>

        {/* Result hero */}
        <div style={{ padding: '1.25rem 1.5rem', borderRadius: 16, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.3)', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <div style={{ fontSize: '0.72rem', color: '#6366f1', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.25rem' }}>
          ...
          {terr.code !== 'fr' && (
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '0.72rem', color: '#64748b', marginBottom: '0.25rem' }}>vs France métropolitaine</div>
              <div style={{ fontSize: '1.8rem', fontWeight: 900, color: ecart > 20 ? '#ef4444' : ecart > 10 ? '#f97316' : '#fbbf24', lineHeight: 1 }}>
                +{ecart.toFixed(1)} %
              </div>
              <div style={{ fontSize: '0.72rem', color: '#64748b' }}>({fmt(prixFinalFR)} € en métro)</div>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: '1.1rem 1.25rem', borderRadius: 14, background: 'rgba(15,23,42,0.75)', border: '1px solid rgba(148,163,184,0.12)', marginBottom: '1.25rem' }}>
          <p style={{ margin: '0 0 1rem', fontSize: '0.78rem', color: '#94a3b8', fontWeight: 600 }}>
            ...
            <div key={i} style={{ marginBottom: '0.85rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.25rem', gap: '0.5rem', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#e2e8f0' }}>{step.label}</span>
                  {step.source && (
                    <a href={step.source.url} target="_blank" rel="noopener noreferrer"
                      style={{ fontSize: '0.6rem', color: '#6366f1', textDecoration: 'none', padding: '1px 6px', borderRadius: 20, background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.25)', whiteSpace: 'nowrap' }}>
                      🔗 {step.source.label}
                    </a>
                  )}
                </div>
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  {step.add > 0 && (
                    <span style={{ fontSize: '0.72rem', color: step.color, fontWeight: 600 }}>+{fmt(step.add)} €</span>
                  )}
                  <span style={{ fontSize: '0.85rem', fontWeight: 800, color: '#f1f5f9' }}>{fmt(step.value)} €</span>
                </div>
              </div>
              {/* Bar */}
              <div style={{ height: 10, background: 'rgba(255,255,255,0.06)', borderRadius: 6, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(step.value / maxValue) * 100}%`, background: `linear-gradient(90deg, ${step.color}88, ${step.color})`, borderRadius: 6, transition: 'width 0.4s ease' }} />
              </div>
              <p style={{ margin: '0.2rem 0 0', fontSize: '0.65rem', color: '#475569', lineHeight: 1.4 }}>{step.note}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
          {[
            { label: 'Fret maritime', value: terr.code === 'fr' ? '0 %' : `+${terr.fretPct} %`, color: '#0ea5e9', src: 'Armateurs de France', url: 'https://www.armateursdefrance.org/' },
            { label: 'Octroi de mer', value: terr.code === 'fr' ? 'N/A' : `${cat.octroi} %`, color: '#a855f7', src: 'EUR-Lex 2022/2', url: 'https://eur-lex.europa.eu/legal-content/FR/TXT/?uri=CELEX:32022R0002' },
            { label: 'Marge distributeur', value: `${margeUsed} %`, color: '#f59e0b', src: 'Autorité de la conc.', url: 'https://www.autoritedelaconcurrence.fr/' },
            { label: 'TVA applicable', value: `${terr.tvaPct} %`, color: '#22c55e', src: 'CGI art. 296', url: 'https://www.legifrance.gouv.fr/' },
          ].map(r => (
            <div key={r.label} style={{ padding: '0.8rem 0.9rem', borderRadius: 10, background: 'rgba(15,23,42,0.6)', border: `1px solid ${r.color}33`, textAlign: 'center' }}>
              <div style={{ fontSize: '1.3rem', fontWeight: 900, color: r.color }}>{r.value}</div>
              <div style={{ fontSize: '0.72rem', color: '#e2e8f0', fontWeight: 600, margin: '0.15rem 0' }}>{r.label}</div>
              <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.6rem', color: '#475569', textDecoration: 'none' }}>🔗 {r.src}</a>
            </div>
          ))}
        </div>
