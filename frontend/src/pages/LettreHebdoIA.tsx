/**
 * LettreHebdoIA — Page archive de la Lettre Hebdomadaire IA
 *
 * Affiche la lettre de la semaine en cours et l'archive des lettres précédentes.
 * Les lettres sont générées chaque lundi par GitHub Actions (script Node.js)
 * et stockées dans Firestore (collection lettre_hebdo_ia).
 */
...
import {
  ...
  getLettreByWeek,
  type LettreHebdo,
} from '../services/lettreHebdoService';
...
function LettreView({ lettre }: { lettre: LettreHebdo }) {
...
export default function LettreHebdoIA() {
  const [current, setCurrent] = useState<LettreHebdo | null>(null);
  const [archive, setArchive] = useState<LettreHebdo[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedWeekId, setSelectedWeekId] = useState<string | null>(null);
  const [loadingArchive, setLoadingArchive] = useState(false);

  useEffect(() => {
    Promise.all([getLatestLettre(), getRecentLettres(12)]).then(([latest, all]) => {
      setCurrent(latest);
      setArchive(all);
      setLoading(false);
    });
  ...
  async function selectWeek(weekId: string) {
    if (current?.weekId === weekId) return;
    setLoadingArchive(true);
    const lettre = await getLettreByWeek(weekId);
