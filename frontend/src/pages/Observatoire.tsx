...
export default function Observatoire() {
  ...
  const isValidData = (value: unknown): value is ObservatoireData => {
    ...
    return (
      ...
      v.panier.every((i) => typeof i.produit === 'string' && typeof i.prix_moyen === 'number')
    );
  };
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-800 text-slate-300 uppercase tracking-wide">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">Prix moyen ({data.devise})</th>
                  </tr>
                </thead>
                <tbody>
                  {data.panier.map((item, idx) => (
                    <tr key={idx} className="border-t border-slate-800 hover:bg-slate-900">
