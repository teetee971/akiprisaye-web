  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <Helmet>
        ...
        <link rel="alternate" hrefLang="fr" href="https://teetee971.github.io/akiprisaye-web/budget-vital" />
        <link rel="alternate" hrefLang="x-default" href="https://teetee971.github.io/akiprisaye-web/budget-vital" />
      </Helmet>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-4">📋 Votre situation</h2>
          ...
          {/* Territory selection */}
          <div className="mt-4">
            <label htmlFor="territory-select" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
              Territoire
            </label>
            <select
              id="territory-select"
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl p-5 border border-blue-200 dark:border-blue-800 flex justify-between items-center">
          <div>
            <p className="text-sm text-blue-700 dark:text-blue-300 font-medium">
              Revenu de référence ({incomeType}) — {selectedTerritory?.flag} {selectedTerritory?.name}
            </p>
        ...
        {budget ? (
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-4">📊 Charges mensuelles incompressibles</h2>
            <div className="space-y-3 mb-6">
              {BUDGET_LINES.map(({ key, label }) => (
                <div key={key} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                  ...
                  <span className="font-semibold text-slate-900 dark:text-white">{budget[key]?.toFixed(2)} €</span>
                </div>
              ))}
            </div>
            <div className="border-t-2 border-slate-300 dark:border-slate-600 pt-4 flex justify-between items-center">
              <span className="text-lg font-bold text-slate-900 dark:text-white">TOTAL DES CHARGES</span>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-2xl p-5">
            <p className="text-yellow-800 dark:text-yellow-200">⚠️ Données non disponibles pour cette combinaison.</p>
          </div>
        )}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="bg-white/50 dark:bg-black/20 rounded-lg p-3 mb-3 font-mono text-sm text-slate-700 dark:text-slate-300">
                  <strong>Calcul :</strong> {incomeRef.toFixed(2)} € − {total.toFixed(2)} € = {resteAVivre.toFixed(2)} €
                </div>
                <p className={`text-sm ${isDeficit ? 'text-red-700 dark:text-red-300' : 'text-green-700 dark:text-green-300'}`}>
                  {isDeficit
                    ? <>Le revenu de référence ne couvre pas les charges essentielles. Il manque <strong>{Math.abs(resteAVivre).toFixed(2)} €</strong> par mois.</>
                    : <>Après les charges essentielles, il reste <strong>{resteAVivre.toFixed(2)} €</strong> pour les autres dépenses.</>
                  }
                </p>
              </div>
            </div>
        ...
        <div className="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
          ...
          <ul className="text-sm text-slate-600 dark:text-slate-400 space-y-1 list-disc list-inside">
            <li>Les charges incluent alimentation, hygiène, transport, énergie/eau et autres coûts incompressibles.</li>
            <li>Le reste à vivre = Revenu de référence − Total des charges.</li>
            <li>Un résultat négatif indique un déficit structurel pour ce profil dans ce territoire.</li>
          </ul>
          <p className="text-xs text-slate-500 dark:text-slate-500 mt-3">
            {budgetData.metadata.source} • Version {budgetData.metadata.version} • {budgetData.metadata.lastUpdate}
          </p>
        </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-4">
          <p className="text-sm text-blue-800 dark:text-blue-200">
            ...
            Chaque situation personnelle est unique. Cet outil ne constitue pas un conseil financier.
          </p>
        </div>
      </div>
    </div>
