...
function PriorityBadge({ priority }: { priority: CroRecommendation['priority'] }) {
  ...
  return (
    <span
      className={`rounded border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${styles[priority]}`}
    >
      {labels[priority]}
    </span>
  );
        ...
        <div className="mx-auto max-w-5xl space-y-8">
                  ...
                  <div key={s.url} className="flex items-center justify-between gap-3 text-xs">
                    ...
                    <div className="flex shrink-0 items-center gap-3 text-right tabular-nums">
                      <span className="text-zinc-500">
                        SEO <ScorePill score={s.seoScore} />
                      </span>
                      <span className="text-zinc-500">
                        Conv <ScorePill score={s.conversionScore} />
                      </span>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mb-2 flex gap-4 text-xs text-zinc-400">
              <span>
                Total clics : <strong className="text-white">{stats.totalClicks}</strong>
              </span>
              <span>
                Meilleure variante : <strong className="text-emerald-400">{topVariant}</strong>
              </span>
              <span>
                Taux :{' '}
                <strong className="text-white">{(stats.conversionRate * 100).toFixed(1)}%</strong>
              </span>
            </div>
            <div className="space-y-3">
              {(['A', 'B', 'C'] as const).map((v) => {
                ...
                const labels = {
                  A: "VOIR L'OFFRE →",
                  B: 'ÉCONOMISEZ X →',
                  C: 'ACHETER AU MEILLEUR PRIX →',
                };
                return (
                  <div key={v}>
                    <div className="mb-1 flex justify-between text-xs text-zinc-400">
                      <span>
                        <span className="font-bold text-white">{v}</span> — {labels[v]}
                      </span>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-sm font-bold text-white">
                📋 Recommandations CRO ({recommendations.length})
              </h2>
              {/* Priority filter */}
                      ...
                      return filtered.map((r, i) => {
                        ...
                        return (
                          <tr key={i} className="align-top">
                            <td className="py-2 pr-4 font-mono text-zinc-400 max-w-[160px] truncate">
                              {r.url}
                            </td>
                            <td className="py-2 pr-4">
                              {score ? (
                                <ScorePill score={score.globalScore} />
                              ) : (
