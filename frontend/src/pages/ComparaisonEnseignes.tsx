    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          />
          <div className="relative w-full rounded-t-[28px] border border-white/10 bg-[#101726] px-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] pt-4 shadow-2xl">
...
export default function ComparaisonEnseignes() {
  ...
  useEffect(() => {
    ...
    const effectiveTerritory =
      territoryFilters.territory !== 'all' ? territoryFilters.territory : selectedTerritory;
    ...
    if (territoryFilters.category !== 'all') {
      obs = obs.filter(
        (o) => !o.metadata?.category || o.metadata.category === territoryFilters.category
      );
    }
  ...
  useEffect(() => {
    ...
    if (territoryFilters.territory !== 'all')
      params.set('territoire_filtre', territoryFilters.territory);
    ...
    selectedProduct,
    selectedTerritory,
    territoryFilters.zone,
    territoryFilters.category,
    territoryFilters.territory,
  ]);

  // Load score enseigne data once on mount
  useEffect(() => {
    computeScoreEnseigne()
      .then((result) => {
        setEnseigneScores(result.global.slice(0, 10));
      })
      .catch(() => {
        /* silently ignore — catalogue-prices.json may be absent */
      });
  }, []);
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <GlassCard>
          <div className="text-center py-8">
            ...
            <p className="text-white/70">Ce module est en cours de déploiement progressif.</p>
          </div>
        </GlassCard>
      </div>
    );
    ...
    <>
      ...
      <div className="container mx-auto max-w-7xl px-4 py-6 pb-28 sm:py-8 sm:pb-10">
        <div className="mb-6 animate-fade-in">
          ...
          >
            <h1 className="text-2xl font-bold text-white drop-shadow">
              Comparaison inter-enseignes
            </h1>
            <p className="text-slate-200 text-sm drop-shadow">
              Données factuelles — aucun classement, aucune recommandation
            </p>
          </HeroImage>
        </div>
        ...
        <div className="mb-6 rounded-2xl border border-blue-500/40 bg-blue-500/15 p-4 text-sm text-blue-100 shadow-[0_8px_24px_rgba(37,99,235,0.12)]">
          ...
          <p className="mt-2">
            Comparaison strictement factuelle entre enseignes, basée sur des observations déclarées
            ou ouvertes. Aucun classement, aucune recommandation, aucune analyse prédictive n'est
            réalisée.
          </p>
        </div>
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2 text-xs text-white/70">
          ...
          <span className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1.5">
            ...
            <span className="font-semibold text-white">
              {selectedTerritory === 'all' ? 'Tous' : selectedTerritory}
            </span>
          </span>
          <span className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1.5">
          ...
          {(selectedTerritory !== 'all' ||
            territoryFilters.zone !== 'all' ||
            territoryFilters.category !== 'all') && (
            <button
              type="button"
              onClick={() => {
            ...
            <GlassCard>
              <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                ...
                <span className="text-sm font-normal text-white/50">
                  (panier catalogue — moins cher en haut)
                </span>
              </h2>
