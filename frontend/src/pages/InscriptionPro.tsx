...
const DOCUMENTS_REQUIS: { type: string; label: string; obligatoire: boolean; detail: string }[] = [
  {
    ...
    obligatoire: true,
    detail:
      "Moins de 3 mois. Pour une micro-entreprise : attestation de l'INSEE ou avis de situation.",
  },
  {
    type: 'identity',
...
function Step1Legal({ form, onChange, onNext }: Step1Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          className={`w-full bg-slate-800 border rounded-lg px-3 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:ring-2 ${
            siretError
              ...
              : validateSiret(form.siret).valid && form.siret
        ...
        {validateSiret(form.siret).valid && form.siret && !siretError && (
          <p className="mt-1 text-xs text-green-400 flex items-center gap-1">
            ...
            SIRET valide — SIREN : {form.siret.slice(0, 9)}
          </p>
        )}
      ...
      {/* Secteur d'activité */}
      <div>
        <label htmlFor="ip-secteur" className="block text-sm font-medium text-gray-300 mb-1">
          Secteur d'activité *
        </label>
        <select
          id="ip-secteur"
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label
            htmlFor="ip-nom-responsable"
            className="block text-sm font-medium text-gray-300 mb-1"
          >
            Nom du responsable *
          </label>
          <input
            id="ip-nom-responsable"
            type="text"
            value={form.nomResponsable}
            onChange={(e) => onChange({ nomResponsable: e.target.value })}
        ...
        <div>
          <label htmlFor="ip-telephone" className="block text-sm font-medium text-gray-300 mb-1">
            Téléphone *
          </label>
          <input
            id="ip-telephone"
      ...
      <div>
        <label htmlFor="ip-email" className="block text-sm font-medium text-gray-300 mb-1">
          Email professionnel *
        </label>
        <input
          id="ip-email"
      ...
      <div>
        <label htmlFor="ip-adresse" className="block text-sm font-medium text-gray-300 mb-1">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label htmlFor="ip-code-postal" className="block text-sm font-medium text-gray-300 mb-1">
            Code postal *
          </label>
          <input
            id="ip-code-postal"
        ...
        <div className="col-span-2">
          <label htmlFor="ip-ville" className="block text-sm font-medium text-gray-300 mb-1">
            Ville *
          </label>
          <input
            id="ip-ville"
      ...
      <div>
        <label htmlFor="ip-territoire" className="block text-sm font-medium text-gray-300 mb-1">
          Territoire *
        </label>
        <select
          id="ip-territoire"
      ...
      <div className="flex gap-3">
        <button
          onClick={onPrev}
          className="px-5 bg-slate-700 hover:bg-slate-600 text-gray-300 rounded-xl font-medium transition-colors flex items-center gap-1"
        >
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-sm text-gray-400">
        Ces informations seront affichées sur votre fiche publique, visible par tous les
        utilisateurs.
      </p>

      <div>
        <label
          htmlFor="ip-description-activite"
          className="block text-sm font-medium text-gray-300 mb-1"
        >
      ...
      <div>
        ...
        <input
          ...
          placeholder="Ex : Les meilleurs prix de l'île !"
          maxLength={80}
      ...
      <div className="bg-amber-900/20 border border-amber-700/30 rounded-xl p-4">
        ...
        <p className="text-xs text-gray-500 mt-3">
          📧 Les documents seront à envoyer par email après la création du compte. Votre compte sera
          activé après vérification (24-48h ouvrées).
        </p>
      </div>
    ...
    <div className="space-y-5">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg ${form.plan === plan.id ? 'bg-blue-600/30 text-blue-300' : 'bg-slate-700/50 text-gray-400'}`}
                >
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {plan.features.slice(0, 3).map((f, i) => (
                <span
                  key={i}
                  className="text-xs bg-slate-700/50 text-gray-400 px-2 py-0.5 rounded-full flex items-center gap-1"
                >
            ...
            {plan.annoncesMax !== null && (
              <div className="mt-2 text-xs text-blue-300 font-medium">
                📋 Quota : {plan.annoncesMax} annonce{plan.annoncesMax > 1 ? 's' : ''} active
                {plan.annoncesMax > 1 ? 's' : ''}
              </div>
            )}
          </button>
        ))}
      ...
      <div className="space-y-3 pt-2">
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            ...
            onChange={(e) => onChange({ acceptesCGU: e.target.checked })}
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            ...
            onChange={(e) => onChange({ acceptesConfidentialite: e.target.checked })}
            className="mt-0.5 w-4 h-4 rounded accent-blue-500 shrink-0"
          />
          <span className="text-sm text-gray-300">
            J'accepte la{' '}
      ...
      <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-4">
        <p className="text-xs text-gray-400">
          <strong className="text-blue-300">Activation du compte :</strong> Après validation de
          votre dossier (SIRET + pièces justificatives), votre compte sera activé sous 24 à 48h
          ouvrées. Vous recevrez un email de confirmation.
        </p>
      </div>
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Ouvrez un compte professionnel et publiez vos prix sur A KI PRI SA YÉ"
        />
      ...
      >
        ...
        <p style={{ margin: '0.25rem 0 0', fontSize: '0.85rem', color: 'rgba(255,255,255,0.75)' }}>
          Accès pro pour enseignes, institutions et collectivités
        </p>
      </HeroImage>
      ...
      <div className="min-h-screen bg-slate-950 py-6 px-4">
        <div className="max-w-2xl mx-auto">
              ...
              return (
                <div key={i} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    ...
                    <span
                      className={`text-xs mt-1 hidden sm:block ${isActive ? 'text-white font-medium' : 'text-gray-500'}`}
                    >
                      {s.label}
                    </span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <div
                      className={`h-0.5 flex-1 mx-2 ${i < step ? 'bg-green-600' : 'bg-slate-700'}`}
                    />
                  )}
                </div>
              );
          ...
          <p className="text-xs text-gray-500 text-center mt-4">
            Déjà un compte pro ?{' '}
            <Link to="/espace-pro" className="text-blue-400 underline">
