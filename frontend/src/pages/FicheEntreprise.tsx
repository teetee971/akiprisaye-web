...
export default function FicheEntreprise() {
  ...
  const coveredTerritories = company.territories
    .map((code) => TERRITORIES[code])
    .filter(Boolean);

  function handleExportHistory() {
    const header = 'Mois,Indice prix\n';
    const rows = company.priceHistory
      .map((v, i) => `${MONTHS[i]},${v}`)
      .join('\n');
            ...
            <div className='flex items-center gap-4'>
              ...
              <div>
                ...
                <div className='flex items-center gap-1 mt-2'>
                  ...
                  <span className='text-xs text-gray-400'>{coveredTerritories.length} territoire(s) couverts</span>
                </div>
              </div>
            </div>
          </div>
            ...
            {showHistory && (
              <>
                <p className='text-xs text-gray-400 mb-3'>
                  Indice moyen des prix (base 100 = Jan {prevYear}) — données publiques reconstituées
                </p>
                ...
                <div className='mt-4 grid grid-cols-3 gap-3'>
                  ...
                  <div className='bg-violet-50 rounded-xl p-3 text-center'>
                    <p className='text-lg font-bold text-violet-700'>
                      {((company.priceHistory[11] - company.priceHistory[0]) / company.priceHistory[0] * 100).toFixed(1)}%
                    </p>
