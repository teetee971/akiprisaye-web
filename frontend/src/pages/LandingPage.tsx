...
import AlertesPrixBanner, {
  WHATSAPP_SUBSCRIBE_URL,
} from '../components/business/AlertesPrixBanner';
...
export default function LandingPage() {
  ...
  const pageUrl = typeof window !== 'undefined' ? window.location.pathname : '/landing';
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="flex flex-col items-center gap-5 px-4 pb-12 pt-16 text-center sm:pt-24">
        ...
        <h1 className="text-3xl font-extrabold leading-tight tracking-tight sm:text-5xl">
          Trouvez le produit le moins cher <span className="text-emerald-400">en 10 secondes</span>
        </h1>
        ...
        <p className="max-w-md text-sm text-zinc-400 sm:text-base">
          Guadeloupe · Martinique · Guyane — Carrefour, E.Leclerc, Super U et plus encore, comparés
          en temps réel.
        </p>
        ...
        <ul
          className="flex flex-col items-center gap-1 text-xs text-zinc-500 sm:flex-row sm:gap-4"
        ...
        {/* Urgency signal */}
        <p className="text-xs text-amber-400/90">⚠️ Les prix changent tous les jours</p>
        ...
        {ctrPct !== null && ctrPct > 0 && (
          <p className="text-xs text-zinc-500">
            Taux de clic actuel :{' '}
            <span className={`font-bold ${ctrPct >= 10 ? 'text-emerald-400' : 'text-amber-400'}`}>
              {ctrPct} %
            </span>
            {ctrPct < 10 && <span className="ml-1 text-zinc-600">(objectif : 10 %)</span>}
          </p>
        )}
      </section>
        ...
        <div className="mx-auto grid max-w-2xl grid-cols-3 gap-4 text-center">
          {PROOF_ITEMS.map(({ value, label }) => (
            <div key={label} className="flex flex-col gap-1">
              <span className="text-2xl font-extrabold text-emerald-400 sm:text-3xl">{value}</span>
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-3">
          {hoursAgo !== null && (
            <span className="rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1 text-xs font-semibold text-amber-400">
              ⏱ Mis à jour il y a{' '}
              {hoursAgo < 1 ? "moins d'1 heure" : `${hoursAgo} heure${hoursAgo > 1 ? 's' : ''}`}
            </span>
          )}
          {comparisonCount > 0 && (
      ...
      {topDeals.length > 0 && (
        <TopDealsSection deals={topDeals} title="🔥 Les meilleures offres en ce moment" limit={6} />
      )}
      ...
      <section className="mx-auto max-w-md px-4 py-10">
        ...
        <p className="mb-6 text-center text-sm text-zinc-400">{DEMO_PRODUCT.name} — Guadeloupe</p>
                  ...
                  {affiliateUrl && (
                    <span
                      className={`rounded-lg px-2 py-1 text-[10px] font-bold ${
                        isBest ? 'bg-emerald-400/20 text-emerald-300' : 'bg-white/8 text-zinc-400'
                      }`}
                    >
        ...
        <p className="mt-4 text-center text-xs text-zinc-600">
          Économisez jusqu'à{' '}
          <span className="font-bold text-emerald-400">{formatEur(DEMO_SAVINGS)}</span> sur ce seul
          produit.
        </p>
      </section>
