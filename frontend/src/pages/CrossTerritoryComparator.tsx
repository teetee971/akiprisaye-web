...
import { TrendingUp, TrendingDown, Award, AlertCircle, Filter, BarChart2, Info } from 'lucide-react';
...
interface TerritoryRow {
  ...
  bestStore: string; observationCount: number;
  surplusPct: number | null; fairness: number | null; month: string;
}
...
export default function CrossTerritoryComparator() {
  ...
  const products = useMemo(() => {
    ...
    for (const m of allMonthly) if (!map.has(m.productKey)) map.set(m.productKey, { key: m.productKey, name: m.productName, category: m.category });
    return Array.from(map.values()).sort((a, b) => a.name.localeCompare(b.name, 'fr'));
  }, [allMonthly]);

  const categories = useMemo(() => [...new Set(products.map((p) => p.category))].sort(), [products]);
  const filteredProducts = useMemo(() => selectedCategory ? products.filter((p) => p.category === selectedCategory) : products, [products, selectedCategory]);
  useEffect(() => { if (filteredProducts.length > 0 && !selectedProduct) setSelectedProduct(filteredProducts[0].key); }, [filteredProducts, selectedProduct]);

  const rows = useMemo<TerritoryRow[]>(() => {
    if (!selectedProduct) return [];
    const hexEntry = allMonthly.find((m) => m.productKey === selectedProduct && m.territory === 'Hexagone' && (m.month === LATEST_MONTH || m.month === FALLBACK_MONTH));
    const hexPrice = hexEntry?.avgPrice ?? null;
    return COMPARE_TERRITORIES.map((t) => {
      const entry = allMonthly.find((m) => m.productKey === selectedProduct && m.territory.toLowerCase() === t.labelFull.toLowerCase() && m.month === LATEST_MONTH)
        ?? allMonthly.find((m) => m.productKey === selectedProduct && m.territory.toLowerCase() === t.labelFull.toLowerCase() && m.month === FALLBACK_MONTH);
      if (!entry) return { code: t.code, label: t.label, flag: t.flag, avgPrice: null, minPrice: null, maxPrice: null, bestStore: '—', observationCount: 0, surplusPct: null, fairness: null, month: '—' };
      const surplusPct = hexPrice != null && hexPrice > 0 ? ((entry.avgPrice - hexPrice) / hexPrice) * 100 : null;
      return { code: t.code, label: t.label, flag: t.flag, avgPrice: entry.avgPrice, minPrice: entry.minPrice, maxPrice: entry.maxPrice, bestStore: entry.enseignes[0] ?? '—', observationCount: entry.observationCount, surplusPct: surplusPct != null ? Math.round(surplusPct * 10) / 10 : null, fairness: hexPrice != null ? fairnessScore(entry.avgPrice, hexPrice) : null, month: entry.month };
    });
  }, [allMonthly, selectedProduct]);

  const rowsWithData = rows.filter((r) => r.avgPrice != null);
  const cheapest = rowsWithData.reduce<TerritoryRow | null>((b, r) => b == null || r.avgPrice! < b.avgPrice! ? r : b, null);
  const mostExpensive = rowsWithData.reduce<TerritoryRow | null>((w, r) => w == null || r.avgPrice! > w.avgPrice! ? r : w, null);
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 py-8">
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="mb-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 dark:text-amber-200">
                Le <strong>score d'équité</strong> mesure l'écart vs le prix hexagonal (100 = parité, 0 = prix double).
                Le <strong>surplus</strong> indique la surtaxe insulaire en %.
              </p>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {COMPARE_TERRITORIES.map((t) => {
                ...
                return (
                  <TerritoryCard key={t.code} code={t.code} label={t.label} flag={t.flag} className="h-24">
                    {rowForT?.avgPrice != null && (
                      <p className="text-white text-xs font-bold drop-shadow">{rowForT.avgPrice.toFixed(2)} €</p>
                    )}
                  </TerritoryCard>
                );
              })}
          ...
          <div className="mb-6 flex flex-wrap gap-3">
            <div>
              <label htmlFor="cross-categorie" className="block text-xs text-slate-500 mb-1">Catégorie</label>
              <select id="cross-categorie" value={selectedCategory} onChange={(e) => { setSelectedCategory(e.target.value); setSelectedProduct(''); }}
            ...
            <div className="flex-1 min-w-48">
              <label htmlFor="cross-produit" className="block text-xs text-slate-500 mb-1">Produit comparé</label>
              <select id="cross-produit" value={selectedProduct} onChange={(e) => setSelectedProduct(e.target.value)}
                ...
                {filteredProducts.map((p) => <option key={p.key} value={p.key}>{p.name}</option>)}
              </select>
            </div>
          </div>
          ...
          {!loading && !error && rowsWithData.length > 0 && (
            <div className="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-3">
                {selectedProductMeta && <CategoryIcon category={selectedProductMeta.category} size="lg" />}
                <div>
                  <p className="text-xs text-slate-500 mb-0.5">Produit sélectionné</p>
                  <p className="font-semibold text-slate-900 dark:text-white text-sm">{selectedProductMeta?.name ?? '—'}</p>
              ...
              {cheapest && (
                <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800 shadow-sm flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-green-100 dark:bg-green-900 flex items-center justify-center text-xl">{cheapest.flag}</div>
                  <div>
                    <div className="flex items-center gap-1 mb-0.5"><Award className="w-3 h-3 text-green-600" /><p className="text-xs text-green-700 dark:text-green-300">Moins cher</p></div>
                    <p className="font-semibold text-green-900 dark:text-green-100">{cheapest.label}</p>
                    <p className="text-sm font-bold text-green-700 dark:text-green-300">{cheapest.avgPrice?.toFixed(2)} €</p>
                  </div>
                </div>
              )}
              {mostExpensive && mostExpensive.code !== cheapest?.code && (
                <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 border border-red-200 dark:border-red-800 shadow-sm flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900 flex items-center justify-center text-xl">{mostExpensive.flag}</div>
                  <div>
                    <div className="flex items-center gap-1 mb-0.5"><TrendingUp className="w-3 h-3 text-red-500" /><p className="text-xs text-red-700 dark:text-red-300">Plus cher</p></div>
                    <p className="font-semibold text-red-900 dark:text-red-100">{mostExpensive.label}</p>
                    <p className="text-sm font-bold text-red-700 dark:text-red-300">{mostExpensive.avgPrice?.toFixed(2)} €</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {loading ? (
            <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow animate-pulse space-y-3">
              {[...Array(6)].map((_, i) => <div key={i} className="h-12 bg-slate-100 dark:bg-slate-700 rounded" />)}
            </div>
          ) : error ? (
                      ...
                      <tr key={row.code}
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            {row.code === cheapest?.code && <span className="text-green-500 text-xs">★</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3 text-right font-semibold text-slate-900 dark:text-white">
                          {row.avgPrice != null ? `${row.avgPrice.toFixed(2)} €` : <span className="text-slate-300">—</span>}
                        </td>
                        <td className="px-4 py-3 text-right text-green-600 dark:text-green-400">{row.minPrice != null ? `${row.minPrice.toFixed(2)} €` : '—'}</td>
                        <td className="px-4 py-3 text-right text-red-500 dark:text-red-400">{row.maxPrice != null ? `${row.maxPrice.toFixed(2)} €` : '—'}</td>
                        <td className="px-4 py-3 text-right">
                          {row.surplusPct != null ? <span className={surplusLabel(row.surplusPct).cls}>{surplusLabel(row.surplusPct).text}</span> : <span className="text-slate-300">—</span>}
                        </td>
                        <td className="px-4 py-3 text-right">
                          {row.fairness != null ? (
                            <div className="flex items-center justify-end gap-2">
                              <div className="w-16 h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full ${scoreBg(row.fairness)}`} style={{ width: `${row.fairness}%` }} />
                              </div>
                              <span className={`font-bold text-xs ${scoreColor(row.fairness)}`}>{row.fairness}</span>
                            </div>
                          ) : <span className="text-slate-300">—</span>}
                        </td>
                        <td className="px-4 py-3 text-slate-500 dark:text-slate-400 text-xs hidden sm:table-cell">{row.bestStore}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {!loading && !error && rowsWithData.length > 1 && (() => {
            const sorted = [...rowsWithData].filter(r => r.avgPrice != null).sort((a,b) => a.avgPrice! - b.avgPrice!);
            if (sorted.length < 2) return null;
            const cheap = sorted[0]; const expensive = sorted[sorted.length-1];
            ...
            return (
              <div className="mt-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-5 border border-blue-200 dark:border-blue-800">
                <h3 className="font-semibold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                  <TrendingDown className="w-5 h-5 text-blue-600" /> Écart maximal inter-territorial
                </h3>
                <p className="text-slate-700 dark:text-slate-300 text-sm">
                  Le même produit coûte <strong className="text-red-600 dark:text-red-400">{delta.toFixed(1)}% plus cher</strong> à{' '}
                  <strong>{expensive.flag} {expensive.label}</strong> ({expensive.avgPrice?.toFixed(2)} €) qu'à{' '}
                  <strong>{cheap.flag} {cheap.label}</strong> ({cheap.avgPrice?.toFixed(2)} €).
                </p>
              </div>
            );
          })()}
