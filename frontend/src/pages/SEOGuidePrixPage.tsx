...
async function getRealHistoricalPrices(
  productSlug: string
): Promise<Array<{ month: string; price: number }>> {
  ...
  const key =
    Object.keys(historique.products).find((k) =>
      productSlug.includes(k.replace('_', '-').split('_')[0])
    ) ?? 'riz_1kg';
...
export default function SEOGuidePrixPage() {
  ...
  useEffect(() => {
    ...
    getRealHistoricalPrices(productSlug).then((data) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-8 pb-24 sm:pb-8">
      ...
      <div className="mx-auto max-w-2xl space-y-5">
        ...
        <div className="rounded-2xl border border-emerald-400/20 bg-gradient-to-br from-emerald-950/40 to-zinc-900/60 p-5">
          ...
          <p className="mt-2 text-sm text-zinc-400">
            Historique des prix, comparaison des enseignes, conseils pratiques pour payer moins
            cher.
          </p>
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-sm font-bold tabular-nums ${p.isBest ? 'text-emerald-400' : 'text-white'}`}
                    >
                      {formatEur(p.price)}
                    </span>
                    {url && (
          ...
          <div className="mt-3 flex gap-4 text-xs text-zinc-600">
            <span>
              Prix moyen : <strong className="text-zinc-400">{formatEur(avgPrice)}</strong>
            </span>
            <span>
              Économie max : <strong className="text-emerald-400">{formatEur(maxSavings)}</strong>
            </span>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
            {history.map(({ month, price }) => (
              <div
                key={month}
                className="rounded-lg border border-white/5 bg-white/[0.01] p-2 text-center"
              >
        ...
        <section className="rounded-xl border border-white/5 bg-white/[0.01] p-5">
          ...
          <div className="space-y-3 text-sm leading-relaxed text-zinc-400">
            ...
            <p>
              L'octroi de mer, taxe locale sur les importations, s'ajoute au prix de base et
              contribue à l'écart avec les prix métropolitains. En {territoryName}, cet écart est
              estimé à environ{' '}
            ...
            <p>
              Pour payer moins cher {productName} en {territoryName}, la stratégie la plus efficace
              est de comparer les enseignes régulièrement. Notre comparateur met à jour les prix
              quotidiennement pour vous permettre de faire le meilleur choix.
            </p>
          </div>
        </section>
      ...
      <ConversionStickyBar
        ...
        territory={territory}
        onCTAClick={() =>
          bestPrice && trackRetailerClick('', bestPrice.retailer, territory, bestPrice.price)
        }
      />
    </div>
  );
