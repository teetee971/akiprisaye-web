...
export default function Avions() {
  ...
  const [results, setResults] = useState<FlightPrice[]>([]);
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (depart && arrivee) {
      const searchResults = searchFlightPrices({ depart, arrivee, mois: mois || undefined });
              ...
              {results.length > 0 ? (
                ...
                <div className="bg-slate-900/50 backdrop-blur-md rounded-xl border border-slate-700/50 p-8 text-center">
                  <p className="text-gray-400">
                    Aucun résultat pour cette recherche. Essayez une autre route.
                  </p>
                </div>
              )}
            </>
