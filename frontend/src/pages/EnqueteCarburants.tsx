...
function DataCard({
  ...
  return (
    <div
      className={`rounded-xl p-4 border ${highlight ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-slate-800 border-slate-700'}`}
    >
      <p className="text-xs text-gray-400 mb-1">{label}</p>
      <p className={`text-2xl font-bold ${highlight ? 'text-yellow-300' : 'text-white'}`}>
        {value}
      </p>
...
function Step({ n, title, children }: { n: number; title: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-4 mb-6">
      <div className="flex-shrink-0 w-8 h-8 rounded-full bg-yellow-500 text-slate-900 flex items-center justify-center font-bold text-sm">
        {n}
      </div>
      <div>
...
const DOM_DATA: {
...
}[] = [
  {
    ...
    taxePct: 47,
    platfond: true,
    note: 'Prix plafonnés arrêté préfectoral mensuel',
  },
  {
    ...
    taxePct: 47,
    platfond: true,
    note: 'Arrêté de prix, révisé chaque 1er du mois',
  },
  {
    territory: 'Guyane',
    flag: '🇬🇫',
    ...
    taxePct: 45,
    platfond: true,
    note: 'Surcoût fret Amazon ; prix plafonnés',
  },
  {
    ...
    taxePct: 46,
    platfond: true,
    note: 'Approvisionnement via Canal de Suez',
  },
  {
    territory: 'Mayotte',
    flag: '🇾🇹',
    ...
    taxePct: 44,
    platfond: true,
    note: 'Arrêté prix depuis 2022 ; réseau restreint',
  },
  {
    territory: 'Saint-Pierre-Miquelon',
    flag: '🇵🇲',
    dept: '975',
    sp95: 1.75,
    diesel: 1.65,
    taxePct: 39,
    platfond: false,
    note: 'Pas TICPE DOM ; taxe locale spécifique',
  },
  {
    ...
    taxePct: 18,
    platfond: false,
    note: 'COM : TVA 0 % ; taxes locales réduites',
  },
  {
    ...
    taxePct: 30,
    platfond: false,
    note: 'COM ; concurrence avec partie néerlandaise',
  },
];
...
const INTERNATIONAL_DATA = [
  ...
  { country: 'Espagne', flag: '🇪🇸', sp95: 1.6, diesel: 1.54, taxePct: 52 },
...
const ACTORS = [
  {
    ...
    impact:
      'Impact direct sur le prix CIF (coût, assurance, fret) qui constitue ~28 % du prix à la pompe.',
  },
  {
    ...
    impact:
      'La volatilité des marchés peut faire varier le prix à la pompe de ±0,10–0,15 € en quelques semaines.',
  },
  {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://teetee971.github.io/akiprisaye-web/enquete-carburants"
        />
        <link
          rel="alternate"
          hrefLang="fr"
      ...
      <div className="max-w-5xl mx-auto px-4 pb-16 pt-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          <DataCard
            label="Part des taxes dans le prix DOM"
            value="40–50 %"
            sub="vs 60 % en métropole"
            highlight
          />
          <DataCard label="Surcoût fret maritime moyen" value="+9 %" sub="selon territoire" />
          <DataCard
            label="Nb. de territoires avec prix plafonnés"
            value="5 DOM"
            sub="GP, MQ, GF, RE, YT"
          />
          <DataCard
            label="Prix moyen SP95 Guadeloupe 2026"
            value="1,59 €"
            sub="Source : DGEC mars 2026"
          />
        </div>

        {/* Disclaimer */}
        <InfoBox color="amber" title="⚠️ Note méthodologique — Observatoire, pas vendeur">
          Toutes les données de cette page sont issues de sources officielles publiques (DGEC,
          INSEE, IEDOM, Cour des Comptes, arrêtés préfectoraux). Les pourcentages sont des moyennes
          indicatives ; les prix varient chaque mois selon les arrêtés préfectoraux. Cette page a
          pour seul but d'informer les citoyens — aucun lien commercial ou affilié.
        </InfoBox>
        ...
        {activeTab === 'chaine' && (
          <div>
            <SectionTitle icon={TrendingUp}>
              De la mer Arabe à votre île : la chaîne complète
            </SectionTitle>

            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
              Le prix que vous payez à la pompe n'est pas fixé au hasard. Il est le résultat d'une
              longue chaîne de valeur qui commence à des milliers de kilomètres de votre île. Voici
              chaque maillon, expliqué simplement.
            </p>

            <Step n={1} title="Extraction du pétrole brut (Moyen-Orient, Afrique, Mer du Nord)">
              Le pétrole brut est extrait principalement en Arabie Saoudite, aux Émirats, en Irak,
              en Russie et en Norvège. Le prix du baril (159 litres) est coté sur les marchés
              mondiaux en dollars américains, sur la référence <strong>Brent</strong> (mer du Nord).
              En mars 2026, le Brent se négocie autour de <strong>80–85 $/baril</strong>. Ce prix
              fluctue chaque jour selon l'offre, la demande et les décisions de l'OPEP+.
            </Step>
            ...
            <Step n={2} title="Transport maritime vers les raffineries européennes">
              ...
              Le Havre (Normandie), Fos-sur-Mer (Marseille), Rotterdam (Pays-Bas). Le coût de ce
              transport s'appelle le <em>fret pétrolier</em> et s'ajoute au prix du baril. Pour les
              DOM, ce coût est ensuite <strong>doublé</strong> : fret Europe → raffinerie, puis
              raffinerie → île.
            </Step>

            <Step n={3} title="Raffinage : transformer le brut en SP95, Diesel, GPL…">
              Une raffinerie distille le pétrole brut pour en extraire différents produits : gaz
              (GPL), essence (SP95, SP98, E10, E85), kérosène, gazole (diesel), fioul, bitume… La
              marge de raffinage représente environ <strong>10 % du prix final</strong>. Elle est
            ...
            <Step n={4} title="Fret spécial DOM-TOM : le 2e voyage en mer">
              ...
              traverser l'Atlantique, l'Océan Indien, ou le Pacifique. Délais : 10 jours (Antilles),
              22 jours (La Réunion), 14 jours (Saint-Pierre-et-Miquelon). Ce "2e voyage" coûte{' '}
              <strong>+6 à +18 %</strong> du prix final selon la distance.
            </Step>

            <Step n={5} title="Stockage, dépôts & distribution locale">
              À l'arrivée dans le port (ex. Jarry en Guadeloupe, Basse-Terre en Martinique), le
              carburant est stocké dans des dépôts pétroliers. Rubis Énergie, Total/TotalEnergies et
              Esso-ExxonMobil contrôlent l'essentiel de ce réseau dans les DOM. Ce maillon (stockage
              + livraison aux stations) représente ~6 % du prix TTC.
            </Step>

            <Step n={6} title="La pompe : taxes + marge stationiste">
              Le prix affiché à la pompe inclut toutes les taxes (TICPE, TVA, octroi de mer) et la
              marge de la station-service (~3–5 %). Dans les DOM, le prix maximum est fixé chaque
              mois par <strong>arrêté préfectoral</strong> — les stationistes ne peuvent pas
              dépasser ce plafond.
            </Step>
            ...
            <div className="bg-slate-900 rounded-xl p-5 mt-6">
              ...
              <p className="text-xs text-gray-600 mt-3">
                * Ces pourcentages sont des estimations moyennes. La DGEC publie mensuellement des
                tableaux de structure des prix carburants par département.
              </p>
            </div>

            <InfoBox
              color="blue"
              title="💡 Pourquoi les DOM paient-ils moins de taxes que la métropole ?"
            >
              En France métropolitaine, la TICPE est plus élevée et la TVA à 20 % s'applique. Dans
              les DOM, la TVA est à 8,5 % et la TICPE est réduite (voire nulle pour certains
              produits). Résultat : la part fiscale est plus faible (~47 % vs ~60 % en métropole),
              mais le surcoût fret et la concentration des distributeurs maintiennent le prix final
              proche (voire supérieur pour des territoires éloignés).
            </InfoBox>
          </div>
        )}
        ...
        {activeTab === 'taxes' && (
          <div>
            ...
            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
              Un Français qui fait le plein paie en moyenne 55–60 % de taxes dans le prix de
              l'essence. Dans les DOM, c'est moins (~47 %), mais chaque taxe a une logique et une
              destination spécifique. Voici toutes les taxes décortiquées.
            </p>

            <Collapse title="🛢️ TICPE — Taxe Intérieure de Consommation sur les Produits Énergétiques">
              <p className="mb-2">
                C'est la principale taxe sur les carburants, anciennement TIPP. Elle est fixée par
                l'État et perçue par les Douanes. En métropole : ~0,65 €/L pour le SP95, ~0,59 €/L
                pour le gazole.
              </p>
              <p className="mb-2">
              ...
              <p className="text-gray-500">
                Source légale : Article 265 du Code des Douanes ; tarifs dans l'annexe du PLF chaque
                année.
              </p>
            </Collapse>
            ...
            <Collapse title="💰 TVA — Taxe sur la Valeur Ajoutée">
              <p className="mb-2">
                La TVA s'applique sur le prix TTC (TICPE incluse). En métropole : 20 %. Dans les DOM
                : <strong>8,5 %</strong> (taux réduit DOM historique). À Saint-Barthélemy et
                Saint-Martin : TVA à 0 % (Collectivités d'Outre-Mer).
              </p>
              <p className="text-gray-500">
            ...
            <Collapse title="🌊 Octroi de mer régional">
              <p className="mb-2">
                ...
                Régions d'Outre-Mer (Conseil Régional). Pour les carburants, il peut représenter 3–7
                % du prix selon le territoire et les délibérations régionales.
              </p>
              <p className="mb-2">
                Exemple : En Guadeloupe, l'octroi de mer sur l'essence SP95 est d'environ 0,07–0,09
                €/L.
              </p>
              <p className="text-gray-500">
            ...
            <Collapse title="⚡ CSPE — Contribution au Service Public de l'Électricité">
              <p className="mb-2">
                Bien que principalement liée à l'électricité, une fraction de la CSPE finance la
                péréquation tarifaire dans les îles (égalisation des prix énergie avec la
                métropole). Pour les carburants, l'impact est indirect via les taxes sur les
                énergies fossiles.
              </p>
            </Collapse>
            ...
            <Collapse title="🚘 Taxe sur les véhicules de société & taxe régionale carburant">
              <p className="mb-2">
                ...
                (~0,01–0,02 €/L supplémentaires). Cette recette finance des politiques régionales de
                transport.
              </p>
            </Collapse>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={row.t} className="border-t border-slate-800 hover:bg-slate-800/40">
                      ...
                      <td className="px-4 py-3 text-center font-semibold text-yellow-300">
                        {row.total}
                      </td>
                    </tr>
                  ))}
            ...
            <p className="text-xs text-gray-600 mt-2">
              Sources : Annexe tarifaire TICPE — PLF 2025 ; DGEC note informelle DOM ; arrêtés
              préfectoraux 2026. Les valeurs sont indicatives et peuvent varier légèrement en
              fonction des délibérations régionales.
            </p>

            <InfoBox color="red" title="🔎 Ce que l'État gagne sur chaque litre en Guadeloupe">
              Sur 1 litre de SP95 vendu ~1,59 € en Guadeloupe, l'État et la Région récupèrent en
              moyenne
              <strong> ~0,73 €</strong> (TICPE ~0,28 € + TVA ~0,12 € + octroi de mer ~0,08 € +
              autres taxes ~0,05 €). Le reste (~0,86 €) rémunère le pétrole brut, le raffinage, le
              transport maritime, la distribution et la marge de la station.
            </InfoBox>
          </div>
        )}
        ...
        {activeTab === 'dom' && (
          <div>
            ...
            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
              Chaque territoire d'outre-mer a ses spécificités : régime fiscal, distance
              d'approvisionnement, présence ou non d'un arrêté préfectoral. Voici les données
              officielles pour chacun.
            </p>
            ...
            <InfoBox
              color="blue"
              title="📌 Pourquoi Saint-Barthélemy est-elle si chère malgré les taxes réduites ?"
            >
              Saint-Barthélemy (COM) bénéficie de TVA à 0 % et d'une TICPE réduite — la part fiscale
              est seulement ~18 %. Mais le prix dépasse 1,99 €/L ! La raison : l'île est très
              éloignée, les volumes importés sont faibles (pas d'économies d'échelle), et le coût de
              distribution ultra-local (petits volumes, terrain difficile) est élevé. C'est la
              preuve que les taxes ne sont pas le seul facteur.
            </InfoBox>

            <SectionTitle icon={Ship}>
              Le facteur distance : pourquoi ça coûte plus cher
            </SectionTitle>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {[
                    {
                      t: 'Guadeloupe',
                      dist: '~7 200 km (Europe)',
                      duree: '10–11 j',
                      surcout: '+6–8 %',
                      port: 'Jarry (Pointe-à-Pitre)',
                    },
                    {
                      t: 'Martinique',
                      dist: '~7 500 km (Europe)',
                      duree: '10–11 j',
                      surcout: '+6–8 %',
                      port: 'Fort-de-France',
                    },
                    {
                      t: 'Guyane',
                      dist: '~8 100 km (Europe)',
                      duree: '12 j',
                      surcout: '+9–11 %',
                      port: 'Dégrad-des-Cannes',
                    },
                    {
                      t: 'La Réunion',
                      dist: '~10 200 km (Rotterdam)',
                      duree: '22–24 j',
                      surcout: '+11–14 %',
                      port: 'Port-Réunion (Le Port)',
                    },
                    {
                      t: 'Mayotte',
                      dist: '~10 500 km (Moyen-Orient)',
                      duree: '18–20 j',
                      surcout: '+10–12 %',
                      port: 'Longoni',
                    },
                    {
                      t: 'Saint-Pierre-Miquelon',
                      dist: '~5 500 km (Canada/France)',
                      duree: '14 j',
                      surcout: '+15–18 %',
                      port: 'Saint-Pierre',
                    },
        ...
        {activeTab === 'plafonds' && (
          <div>
            ...
            <p className="text-gray-300 text-sm mb-4 leading-relaxed">
              Dans les 5 Départements d'Outre-Mer (Guadeloupe, Martinique, Guyane, La Réunion,
              Mayotte), le prix des carburants n'est <strong>pas libre</strong>. Il est encadré par
              l'État via des arrêtés préfectoraux mensuels. Voici tout ce qu'il faut savoir.
            </p>

            <InfoBox color="green" title="✅ Qu'est-ce qu'un arrêté préfectoral de prix ?">
              C'est un texte juridique signé par le Préfet du département qui fixe, pour le mois
              suivant, le <strong>prix maximum de vente</strong> au détail de chaque type de
              carburant dans tout le département. Les stationistes qui vendent au-dessus de ce prix
              s'exposent à des sanctions de la DGCCRF.
            </InfoBox>
            ...
            <Step n={2} title="La révision mensuelle">
              L'arrêté est révisé le <strong>1er de chaque mois</strong> (parfois plus souvent en
              cas de choc pétrolier). Si le prix du baril monte fortement, le préfet peut décider de
              ne pas répercuter 100 % de la hausse (amortisseur social). Si le baril baisse, la
              répercussion est normalement automatique.
            </Step>

            <Step
              n={3}
              title="Pourquoi le prix plafond peut-il rester élevé même quand le baril baisse ?"
            >
              Plusieurs raisons :
              <ul className="mt-2 space-y-1 list-disc ml-4">
                <li>
                  Le fret maritime est facturé à la date de commande, pas de livraison (décalage ~6
                  semaines)
                </li>
                <li>Les stocks existants ont été achetés au prix antérieur</li>
                <li>
                  L'État peut maintenir un niveau de TICPE pour équilibrer ses recettes fiscales
                </li>
            ...
            <Collapse title="📄 Textes de référence pour les arrêtés préfectoraux">
              <ul className="space-y-2 mt-2">
                <li>
                  • <strong>Guadeloupe :</strong> Arrêtés de la Préfecture de Région publiés sur
                  prefecture.guadeloupe.gouv.fr — rubrique "Arrêtés"
                </li>
                <li>
                  • <strong>Martinique :</strong> Préfecture de Martinique — martinique.pref.gouv.fr
                </li>
                <li>
                  • <strong>Guyane :</strong> Préfecture de Guyane — guyane.pref.gouv.fr
                </li>
                <li>
                  • <strong>La Réunion :</strong> Préfecture de La Réunion — reunion.pref.gouv.fr
                </li>
                <li>
                  • <strong>Mayotte :</strong> Préfecture de Mayotte — mayotte.pref.gouv.fr
                </li>
              </ul>
            </Collapse>
            ...
            <Collapse title="🔍 Peut-on contester un arrêté de prix ?">
              <p className="mb-2">
                Oui. Les associations de consommateurs (UFC-Que Choisir, Consommag en Guadeloupe,
                ASSECO en Martinique) ont le droit de saisir le Tribunal Administratif si elles
                estiment que la formule de calcul est incorrecte ou que des éléments de coûts sont
                surévalués.
              </p>
              <p>
                C'est aussi le rôle de l'observatoire citoyen comme A KI PRI SA YÉ : surveiller que
                les prix pratiqués respectent bien les arrêtés, et alerter si une station vend
                au-dessus du plafond légal.
              </p>
            </Collapse>

            <InfoBox color="amber" title="⚠️ Ce que les prix plafonnés ne garantissent pas">
              Un prix plafonné fixe un maximum, pas un minimum. En théorie, un stationiste peut
              vendre moins cher. En pratique, avec l'oligopole local (peu d'acteurs), le prix
              plafond devient souvent le prix unique pratiqué par tous — ce qui réduit la
        ...
        {activeTab === 'international' && (
          <div>
            <SectionTitle icon={Globe}>
              Comparatif mondial : où le carburant est-il le plus cher ?
            </SectionTitle>

            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
              Pour comprendre le prix de votre carburant, il faut le situer dans le contexte
              mondial. La différence entre pays est principalement due aux taxes, pas au prix du
              brut (identique pour tous au niveau mondial).
            </p>
            ...
            <InfoBox
              color="blue"
              title="🧮 Pourquoi le Venezuela vend-il le carburant à moins de 0,01 €/L ?"
            >
              Le Venezuela subventionne massivement ses carburants grâce à ses réserves pétrolières.
              C'est une politique de subvention nationale totale : l'État vend à perte pour des
              raisons sociales et politiques. Le pays est le premier producteur de réserves prouvées
              au monde. Cette politique coûte des milliards de dollars/an à l'État vénézuélien.
            </InfoBox>

            <InfoBox
              color="amber"
              title="🏔️ Pourquoi la Norvège, premier producteur européen de pétrole, paye-t-elle parmi les plus cher ?"
            >
              La Norvège est un cas classique d'enseignement économique. Elle est 3e exportateur
              mondial de gaz et grand producteur de pétrole, mais ses citoyens paient l'essence très
              chère. Raison : ses taxes sont très élevées (67 %) pour décourager l'usage de la
              voiture thermique et financer la transition énergétique. Elle vend son pétrole au prix
              du marché mondial et reverse les recettes à son fonds souverain (plus de 1 500
        ...
        {activeTab === 'acteurs' && (
          <div>
            <SectionTitle icon={Landmark}>
              Qui fixe le prix du carburant ? Les acteurs décortiqués
            </SectionTitle>

            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
              Le prix à la pompe est la résultante de décisions prises par de nombreux acteurs, à
              des niveaux très différents — du désert saoudien à la préfecture de Basse-Terre. Voici
              chaque acteur, son rôle, et son impact réel.
            </p>
            ...
            <InfoBox
              color="red"
              title="🏭 L'oligopole local : le problème que personne ne dit clairement"
            >
              Dans les Antilles, La Réunion et la Guyane, le secteur pétrolier aval (dépôts,
              distribution, stations) est contrôlé par très peu d'acteurs :{' '}
              <strong>Rubis Énergie</strong> (ex-Vitogaz, Antilles Gaz), <strong>Total</strong>, et{' '}
              <strong>Esso/ExxonMobil</strong>. Cette concentration est légale mais réduit la
              pression concurrentielle. La DGCCRF a ouvert plusieurs enquêtes sur les marges
              pratiquées dans les DOM. Sans les prix plafonnés, les marges pourraient être bien plus
              élevées.
            </InfoBox>
          </div>
        )}
        ...
        {activeTab === 'sources' && (
          <div>
            <SectionTitle icon={BookOpen}>
              Sources officielles & méthode de l'Observatoire
            </SectionTitle>

            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
              A KI PRI SA YÉ est un observatoire citoyen. Notre mission est d'apporter de la
              transparence sur les prix dans les Outre-mer. Voici l'ensemble des sources utilisées
              dans ce dossier et la méthode appliquée pour vérifier les chiffres.
            </p>
            ...
            <InfoBox color="blue" title="🎯 Notre principe : Observer, pas vendre">
              ...
              d'affiliation avec une enseigne pétrolière. Nos seuls revenus proviennent des
              abonnements citoyens et des licences institutionnelles. Cela garantit notre
              indépendance éditoriale.
            </InfoBox>

            <h3 className="font-semibold text-white mt-6 mb-3">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-start gap-2 text-sm text-gray-300">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-green-500/20 text-green-400 text-xs flex items-center justify-center font-bold mt-0.5">
                    ✓
                  </span>
                  {m}
                </div>
            ...
            <InfoBox
              color="green"
              title="💬 Vous avez des données à partager ou une correction à signaler ?"
            >
              Si vous êtes agent de préfecture, stationiste, ou citoyen avec des données officielles
              (arrêtés, relevés de prix, etc.), contactez-nous via notre formulaire de contact. Nous
              vérifions et intégrons les contributions dans notre base de données.
            </InfoBox>
          </div>
        )}
        ...
        <div className="mt-12 bg-gradient-to-r from-orange-900/30 to-slate-900 border border-orange-500/20 rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-400">
              Utilisez notre comparateur pour trouver la station la moins chère et naviguer
              directement vers elle avec le GPS.
            </p>
          </div>
        ...
        <p className="mt-6 text-xs text-gray-700 text-center">
          Observatoire A KI PRI SA YÉ — Dossier carburants v2.0 — Mars 2026. Ce contenu est fourni à
          titre informatif uniquement. Voir{' '}
