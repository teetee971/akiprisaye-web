...
export default function SeoLoopDashboardPage() {
  ...
  return (
    <>
      <SEOHead
        title="SEO Loop Dashboard"
        description="Analyse du loop SEO et actions prioritaires."
        noIndex
      />
        ...
        <div className="mx-auto max-w-4xl space-y-8">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">
              Analyse des actions SEO et suggestions de duplication
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex flex-col gap-1">
              <label
                htmlFor="seoloop-filter-pagetype"
              ...
              >
                ...
                {(['product', 'category', 'comparison', 'inflation', 'pillar'] as const).map(
                  (t) => (
            ...
            <div className="flex flex-col gap-1">
              <label
                htmlFor="seoloop-filter-priority"
                className="text-[10px] font-bold uppercase tracking-wide text-zinc-500"
              >
                Priorité
              </label>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
            <h2 className="mb-4 text-sm font-bold text-white">
              Actions SEO ({filteredActions.length})
            </h2>
          ...
          {allSuggestions.length > 0 && (
            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
              <h2 className="mb-4 text-sm font-bold text-white">
                Suggestions de duplication ({allSuggestions.length})
              </h2>
              <div className="space-y-2">
                {allSuggestions.map((s, i) => (
                  <div
                    key={i}
                    className="flex flex-wrap items-center gap-2 rounded-lg bg-white/[0.02] p-3"
                  >
                    <span
                    ...
                    <span className="font-mono text-xs text-emerald-400">
                      {s.suggestedUrl.length > 35
                        ? `${s.suggestedUrl.slice(0, 35)}…`
                        : s.suggestedUrl}
                    </span>
