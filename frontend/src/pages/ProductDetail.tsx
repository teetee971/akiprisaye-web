...
export default function ProductDetail() {
  ...
  if (!product) {
    return (
      <div className="max-w-3xl mx-auto p-4">
        Produit introuvable.{' '}
        <Link to="/comparateur" className="text-blue-600">
          Retour
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-4">
      <Link to="/comparateur" className="text-blue-600 flex items-center gap-1 text-sm">
        ← Retour comparateur
      </Link>
          ...
          {user && (
            ...
            >
              ...
              <span className="text-xs font-medium">
                {isFollowing ? 'Alerte active' : 'Suivre le prix'}
              </span>
            </button>
          )}
        </div>
