        ...
        <div style={{ maxWidth: 780, margin: '0 auto' }}>
          ...
          >
            <div style={{ display: 'flex', gap: '0.4rem' }}>
              {[
                { id: 'tous', label: '🔄 Tous' },
                ...LIEUX.map((l) => ({ id: l.id, label: `${l.emoji} ${l.label}` })),
              ].map((f) => (
                <button
                  key={f.id}
                  onClick={() => setFilterLieu(f.id as FridgeItem['lieu'] | 'tous')}
                  style={{
                    padding: '0.35rem 0.75rem',
                    borderRadius: 20,
                    border: `1px solid ${filterLieu === f.id ? 'rgba(99,102,241,0.5)' : 'rgba(148,163,184,0.2)'}`,
                    background: filterLieu === f.id ? 'rgba(99,102,241,0.15)' : 'transparent',
                    color: filterLieu === f.id ? '#a5b4fc' : '#64748b',
                    fontSize: '0.73rem',
                    cursor: 'pointer',
                  }}
                >
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'dlc' | 'nom')}
              style={{
          ...
          {showForm && (
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    value={form.lieu}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, lieu: e.target.value as FridgeItem['lieu'] }))
                    }
                    style={{
                      width: '100%',
                      padding: '0.45rem 0.6rem',
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button
                  onClick={addItem}
                  disabled={!form.nom || !form.dlc}
                  style={{
                    padding: '0.45rem 1.1rem',
                    borderRadius: 8,
                    background:
                      !form.nom || !form.dlc ? 'rgba(16,185,129,0.3)' : 'rgba(16,185,129,0.75)',
                    color: '#fff',
                    fontSize: '0.82rem',
                    fontWeight: 700,
                    border: 'none',
                    cursor: !form.nom || !form.dlc ? 'not-allowed' : 'pointer',
                  }}
                >
                  ✅ Enregistrer
                </button>
          ...
          {displayed.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem 1rem', color: '#334155' }}>
              ...
              <p style={{ margin: 0, fontSize: '0.88rem' }}>
                Votre liste est vide — ajoutez vos produits !
              </p>
            </div>
          ) : (
                    ...
                    <div style={{ flex: 1, minWidth: 160 }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: '0.65rem', color: '#475569' }}>
                          {lieu.emoji} {lieu.label}
                        </span>
                      </div>
                      <div style={{ fontSize: '0.72rem', color: '#64748b', marginTop: '0.15rem' }}>
                        {item.categorie} · {item.quantite} · {formatDiff(item.dlc)} (
          ...
          >
            <p style={{ margin: 0, fontSize: '0.72rem', color: '#4ade80', lineHeight: 1.6 }}>
              💡 <strong>Astuce :</strong> Dans les DOM, un ménage gaspille en moyenne{' '}
              <strong>150 € d'aliments/an</strong> — soit 1,5× plus qu'en métropole en valeur
              relative au budget alimentaire (ADEME 2023). Chaque produit consommé avant péremption
              est une victoire contre la vie chère !
            </p>
          </div>
