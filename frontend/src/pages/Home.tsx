...
const Home = () => {
  ...
  const promos = [
    { id: 1, title: "OFFRES SUPER U", subtitle: "Grand Ouverture v4.6.19", img: "https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&q=80&w=400", action: () => navigate('/flyer') },
    { id: 2, title: "ACTUALITÉS", subtitle: "Vidéo Souveraine Réparée", img: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=400", action: () => navigate('/connexion') },
  ...
  return (
    <div id="root-container" className="min-h-screen bg-[#0f172a] text-white pb-24">
      {/* Header */}
      ...
      <div className="mb-10">
        ...
        <div className="flex gap-4 overflow-x-auto px-6 pb-4 scrollbar-hide snap-x">
          {promos.map(promo => (
            <div key={promo.id} onClick={promo.action} className="relative flex-none w-72 aspect-video rounded-3xl overflow-hidden border border-slate-700/50 snap-center cursor-pointer shadow-2xl">
      ...
      {/* RECHERCHE (ENCAPSULÉE DANS UN FORM POUR LES TESTS) */}
      ...
      <div className="px-6 flex gap-2 overflow-x-auto scrollbar-hide">
        {Object.keys(territoryNames).map(t => (
          <button key={t} onClick={() => setTerritory(t)} className={`px-5 py-2 rounded-xl font-bold text-xs transition-all ${territory === t ? 'bg-blue-600' : 'bg-slate-800 text-slate-500'}`}>
            {t}
          </button>
        ))}
      </div>
