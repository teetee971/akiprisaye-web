...
export default function AlertesPredictives() {
  ...
  const now = new Date();
  const startDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + (optimalWindow?.start ?? 1) - 1);
  const endDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + (optimalWindow?.end ?? 5) - 1);
  ...
  return (
    <div className='max-w-5xl mx-auto px-4 py-8 space-y-8'>
        ...
        <div className='overflow-x-auto'>
          <svg viewBox={`0 0 ${chartWidth} ${chartHeight + 20}`} className='w-full max-w-2xl'>
            ...
            {[1, 10, 20, 30].map((day) => {
              ...
              return (
                <text key={day} x={x} y={chartHeight + 14} textAnchor='middle' fontSize={10} fill='#9ca3af'>
                  J+{day}
                </text>
              );
            })}
        ...
        {optimalWindow && (
          <div className='mt-4 flex items-center gap-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3'>
            ...
            <p className='text-sm text-green-800 dark:text-green-300'>
              <strong>Fenêtre optimale :</strong> du{' '}
              {startDate.getDate()} {MONTHS[startDate.getMonth()]} au{' '}
              {endDate.getDate()} {MONTHS[endDate.getMonth()]} — économie estimée :{' '}
              <strong>-{optimalWindow.saving}%</strong>
            </p>
          </div>
        )}
      </section>
      ...
      {/* Active alerts */}
      <section className='bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6'>
        <h2 className='text-lg font-semibold text-gray-900 dark:text-white mb-4'>Mes alertes actives</h2>
        ...
        {alerts.length === 0 ? (
          ...
          <div className='space-y-2 mb-6'>
            {alerts.map((a) => (
              ...
              >
                <div>
                  <p className='text-sm font-medium text-gray-900 dark:text-white'>{a.productName}</p>
        ...
        <div className='border-t border-gray-200 dark:border-gray-700 pt-4'>
          <h3 className='text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3'>Ajouter une alerte</h3>
