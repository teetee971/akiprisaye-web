...
const FUEL_DESIGN: Record<
  string,
  {
    label: string;
    abbr: string;
    from: string;
    to: string; // gradient start/end
...
function FuelChip({
  ...
  return onClick ? (
    <button type="button" onClick={onClick} aria-pressed={active} style={style}>
      <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.06em', opacity: 0.85 }}>
        {d.abbr}
      </span>
      {price !== undefined && (
...
function StationCard({ station, fuelFilter, isCheapest }: StationCardProps) {
  ...
  const fuels = station.fuels.filter((f) => fuelFilter === 'TOUS' || f.fuelType === fuelFilter);
  ...
  return (
    <div
      ...
      style={{
        ...
        border: isCheapest
          ? '1.5px solid rgba(52,211,153,.5)'
          : '1.5px solid rgba(148,163,184,.12)',
...
export default function StationsProximite() {
  ...
  const [gpsStatus, setGpsStatus] = useState<'idle' | 'loading' | 'found' | 'denied' | 'error'>(
    'idle'
  );
  ...
  const cheapestId = useMemo(() => {
    ...
    for (const st of nearbyStations) {
      ...
      if (match && match.pricePerLiter < min) {
        min = match.pricePerLiter;
        winner = st.id;
      }
    }
    return winner || null;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Trouvez les stations-service les plus proches avec les prix réglementés du carburant en DOM-TOM. GPS, carte interactive, itinéraire."
        />
        <link
          rel="canonical"
          href="https://teetee971.github.io/akiprisaye-web/stations-proximite"
        />
      </Helmet>
      ...
      <div
        ...
        style={{ background: 'linear-gradient(160deg,#0a0f1e 0%,#0f172a 60%,#1a0a2e 100%)' }}
      >
        {/* ── Hero header ── */}
        <div className="relative overflow-hidden px-4 pt-10 pb-8">
          ...
          />
          <div
            className="absolute top-4 -left-8 w-48 h-48 rounded-full opacity-15"
          ...
          <div className="relative max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[.2em] text-orange-400 mb-0.5">
                  DOM-TOM
                </p>
        ...
        <div className="max-w-2xl mx-auto px-4 pb-12 space-y-4">
          ...
          >
            ...
            {/* Fuel type filter */}
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-500 mb-2">
                Carburant
              </p>
              <div className="flex flex-wrap gap-2">
            ...
            {userPos && (
              <div
                ...
                style={{ borderColor: 'rgba(148,163,184,.08)' }}
              >
                <span className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-500">
                  Trier
                </span>
                <div
                  className="flex rounded-xl overflow-hidden"
                  style={{ border: '1px solid rgba(100,116,139,.25)' }}
                >
          ...
          >
            {dataLoading ? (
              <div
                className="h-80 flex flex-col items-center justify-center gap-3"
                    ...
                    <Marker key={st.id} position={[st.location.lat, st.location.lng]} icon={icon}>
                      <Popup minWidth={210}>
                        <div style={{ fontFamily: 'system-ui', padding: 2 }}>
                          <div style={{ fontWeight: 800, fontSize: '0.85rem', marginBottom: 3 }}>
                            ⛽ {st.name}
                          </div>
                          <div
                            style={{
                              fontSize: '0.7rem',
                              color: '#64748b',
                              marginBottom: st.distance !== undefined ? 3 : 6,
                            }}
                          >
          ...
          {!userPos && gpsStatus === 'idle' && (
            <div className="text-center py-10 space-y-3">
              ...
              <p className="text-base font-bold text-white">
                Activez votre GPS pour voir
                <br />
                les stations autour de vous
              </p>
              ...
              {nearbyStations.length === 0 && (
                ...
                >
                  <p className="text-slate-400 text-sm">
                    Aucune station répertoriée dans ce rayon.
                  </p>
                  <p className="text-xs text-slate-600">
                    En DOM-TOM les prix étant réglementés, toutes les stations affichent les mêmes
                    tarifs.
                  </p>
                </div>
              )}
