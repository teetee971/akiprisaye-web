...
export default function PromosPage() {
  ...
  const [territory, setTerritory] = useState<TerritoryCode>(
    effectiveSelection.territory as TerritoryCode
  );
  ...
  const basePromos = useMemo(
    () =>
      getPromos({
        territory,
        storeId: onlyMyStore ? effectiveSelection.storeId : undefined,
        mode: effectiveSelection.serviceMode,
      }),
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 text-slate-100">
        ...
        <p className="text-sm text-slate-400 mb-6">
          Offres locales en mode {effectiveSelection.serviceMode ?? 'inStore'} · territoire{' '}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
          ...
          <select
            className="bg-slate-800 border border-slate-700 rounded-lg p-2"
            value={category}
            onChange={(e) => setCategory(e.target.value as PromoCategory | 'all')}
          >
          ...
          <select
            className="bg-slate-800 border border-slate-700 rounded-lg p-2"
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as PromoSort)}
          >
        ...
        <label className="inline-flex items-center gap-2 mb-6 text-sm text-slate-200">
          ...
          />
          Uniquement mon magasin{' '}
          {effectiveSelection.storeId
            ? `(${storeLabels[effectiveSelection.storeId] ?? effectiveSelection.storeId})`
            : '(non sélectionné)'}
        </label>
            ...
            <article key={promo.id} className="rounded-xl border border-slate-700 bg-slate-900 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs text-blue-300 uppercase tracking-wide">
                    {categoryLabels[promo.category]}
                  </p>
                  <h2 className="font-semibold text-base mt-1">{promo.title}</h2>
                  <p className="text-sm text-slate-400">
                    {promo.brand ?? 'Marque locale'} ·{' '}
                    {storeLabels[promo.storeId ?? ''] ?? 'Magasin du territoire'}
                  </p>
                </div>
                {promo.discountPct ? (
                  <span className="text-sm font-bold text-emerald-300">-{promo.discountPct}%</span>
                ) : null}
              </div>
              ...
              <div className="text-sm mt-3 mb-4">
                ...
                <p className="text-slate-400">
                  Valable du {promo.validFrom ?? 'N/A'} au {promo.validTo ?? 'N/A'}
                </p>
              </div>

              <button
                type="button"
                className="inline-flex px-3 py-2 rounded bg-blue-600 hover:bg-blue-500 text-sm"
              >
                Créer alerte
              </button>
            </article>
