...
import type { FuelPricePoint, FuelType, FuelComparisonResult } from '../types/fuelComparison';
...
const FuelComparator: React.FC = () => {
  ...
  const territoryStats = useMemo(() => {
    ...
    return TERRITORIES.map((t) => {
      ...
      if (!prices.length)
        return { ...t, min: null as number | null, avg: null as number | null, count: 0 };
  ...
  const sortedPrices = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortBy === 'station')
        return a.fuelPrice.station.name.localeCompare(b.fuelPrice.station.name);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <Helmet>
        ...
        <meta
          name="description"
          content="Prix des carburants en direct dans tous les territoires et départements d'outre-mer : SP95, SP98, E10, Diesel, GPL. Données officielles prix-carburants.gouv.fr."
        />
        <link
          rel="canonical"
          href="https://teetee971.github.io/akiprisaye-web/comparateur-carburants"
        />
        <link
          rel="alternate"
          hrefLang="fr"
        ...
        <div className="mb-6">
          ...
          >
            ...
            <p className="text-yellow-100 text-sm drop-shadow">
              Prix en direct dans tous les territoires d&apos;outre-mer &mdash; donn&eacute;es
              officielles prix-carburants.gouv.fr
            </p>
        ...
        {viewMode === 'single' && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                <label htmlFor="fc-territoire" className="block text-sm text-gray-400 mb-1">
                  Territoire
                </label>
                <select
                  id="fc-territoire"
                  ...
                  onChange={(e) => {
                    setSelectedTerritory(e.target.value as Territory);
                    setFilterCity('');
                  }}
                ...
                >
                  {TERRITORIES.map((t) => (
                    <option key={t.code} value={t.code}>
                      {t.flag} {t.name} (dept. {t.dept})
                    </option>
                  ))}
                </select>
              </div>
              <div>
                <label htmlFor="fc-carburant" className="block text-sm text-gray-400 mb-1">
                  Type de carburant
                </label>
                <select
                  id="fc-carburant"
              ...
              <div>
                <label htmlFor="fc-ville" className="block text-sm text-gray-400 mb-1">
                  Filtrer par ville
                </label>
                <input
                  id="fc-ville"
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              {gpsStatus === 'found' && nearestStation && (
                ...
                >
                  ...
                  Station proche&nbsp;: {nearestStation.fuelPrice.station.name} &middot;{' '}
                  {formatPrice(nearestStation.fuelPrice.pricePerLiter)}/L
                </a>
              )}
              {gpsStatus === 'error' && (
              ...
              {liveLoading && (
                <span className="text-xs text-blue-400 flex items-center gap-1">
                  <Globe className="w-3 h-3 animate-spin" /> R&eacute;cup&eacute;ration des prix en
                  direct&hellip;
                </span>
              )}
            </div>
              ...
              <>
                ...
                {comparisonResult.rankedPrices.length > 0 &&
                  (() => {
                    ...
                    const navUrl = getStationNavUrl(
                      cheapest.fuelPrice.station.name,
                      cheapest.fuelPrice.station.city,
                      lat,
                      lng
                    );
                    return (
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              {cheapest.fuelPrice.station.city} &middot;{' '}
                              {cheapest.fuelPrice.station.brand}
                            </p>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 my-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <ShareButton
                      title="Comparateur Carburants"
                      description="Comparez les prix des carburants dans votre territoire"
                    />
                  </div>
                </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right font-semibold">
                              {formatPrice(item.fuelPrice.pricePerLiter)}
                            </td>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-orange-300">
                      Enquête complète : d&apos;où vient le prix ?
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      Taxes, marges, prix plafonn&eacute;s, comparatif international&hellip;
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-amber-300">
                      Conf&eacute;rence expert — Anatomie du prix
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      9 diapositives · Brent, SARA, fiscalit&eacute;, plafonnement, comparaison
                      mondiale&hellip;
                    </p>
                  </div>
        ...
        {viewMode === 'compare' && (
          <>
            <div className="flex flex-wrap items-end gap-4 mb-6">
              <div>
                <label htmlFor="fc-carburant-compare" className="block text-sm text-gray-400 mb-1">
                  Type de carburant
                </label>
                <select
              ...
              <p className="text-xs text-gray-500">
                Cliquez sur un territoire pour voir le d&eacute;tail et naviguer GPS vers les
                stations.
              </p>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {territoryStats.map((t) => (
                  <button
                    key={t.code}
                    onClick={() => {
                      setSelectedTerritory(t.code as Territory);
                      setViewMode('single');
                    }}
                    ...
                    {t.min !== null ? (
                      <>
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          moy.&nbsp;{t.avg !== null ? formatPrice(t.avg) : '—'} &middot; {t.count}{' '}
                          station{t.count > 1 ? 's' : ''}
                        </p>
                      </>
                    ) : (
                      <p className="text-xs text-gray-600 italic mt-1">
                        Donn&eacute;es en attente&hellip;
                      </p>
                    )}
