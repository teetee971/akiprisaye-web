...
import {
  Droplet, TrendingUp, Globe, DollarSign, BarChart2,
  AlertTriangle, Info, ChevronRight, ChevronDown,
  FileText, MapPin, BookOpen, ExternalLink,
  ...
  return (
    <div>
      ...
      <p className="text-gray-300 text-sm mb-6 leading-relaxed">
        ...
        Dans les territoires d'outre-mer français, ce droit reste fragile : infrastructures vieillissantes,
        contaminations historiques, tarifs élevés et inégalités d'accès persistent malgré des décennies d'investissements.
        Cette enquête documente la réalité du service d'eau dans les cinq DROM avec des données officielles.
      </p>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        <DataCard label="Écart tarifaire moyen DOM/Métropole" value="+40 à +120%" sub="selon territoire et service" highlight />
        <DataCard label="Taux de fuite réseau Guadeloupe" value="~60%" sub="vs 20% en métropole" />
        <DataCard label="Pop. sans eau potable (Mayotte)" value="~15–20%" sub="Source : ARS Mayotte 2023" />
      ...
      <InfoBox color="amber" title="⚠️ Note méthodologique">
        Les données présentées proviennent de sources officielles publiques : ARS, Offices de l'eau DOM,
        Cour des Comptes, DGALN/SISPEA, UFC-Que Choisir. Les tarifs cités sont indicatifs et peuvent
        varier selon la commune, l'intercommunalité et le contrat en cours. Cette page vise à informer,
        pas à remplacer une consultation juridique ou technique.
      </InfoBox>
      ...
      <div className="space-y-3 mb-6">
        {[
          ...
          { t: 'Guadeloupe 🇬🇵',  v: 95, note: 'Réseau raccordé mais interruptions fréq.', c: '#60a5fa' },
          ...
          <div key={row.t} className="bg-slate-800/50 rounded-xl p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-bold" style={{ color: row.c }}>{row.v}%</span>
            </div>
      ...
      <div className="space-y-3 mb-6">
        {[
          { year: '2006', title: 'Loi sur l\'Eau et les Milieux Aquatiques (LEMA)', note: 'Renforcement du droit d\'accès à l\'eau, organisation des services.', c: '#22d3ee' },
          { year: '2010', title: 'Résolution ONU — Droit à l\'eau potable', note: 'L\'accès à l\'eau est reconnu droit humain fondamental (résolution 64/292).', c: '#60a5fa' },
          { year: '2013', title: 'Début de la crise Guadeloupe', note: 'Le Syndicat Mixte Ouvert (SMO) dénonce l\'état catastrophique du réseau. Signalement à l\'État.', c: '#f59e0b' },
          { year: '2020', title: 'Rapport Cour des Comptes', note: 'Constat sévère sur la gestion des services d\'eau dans les DOM : sous-investissement chronique.', c: '#ef4444' },
          { year: '2021', title: 'Directive UE 2020/2184', note: 'Nouvelle directive sur l\'eau potable : normes renforcées, obligation de transparence tarifaire.', c: '#a78bfa' },
          { year: '2022', title: 'Plan Urgence Eau Guadeloupe', note: 'L\'État débloque 40 M€ d\'urgence pour les réseaux guadeloupéens les plus dégradés.', c: '#22d3ee' },
          { year: '2023', title: 'Plan Eau national (53 mesures)', note: 'Premier ministre : sobriété, réutilisation des eaux usées, soutien spécifique aux outre-mer.', c: '#4ade80' },
          { year: '2024', title: 'Crise hydrique Mayotte', note: 'Restrictions d\'eau : alimentation 1 jour sur 2. ARS et État déclenchent le plan de crise.', c: '#ef4444' },
        ].map((ev, i) => (
          <div key={ev.year} className="flex gap-3">
            <div className="flex flex-col items-center w-10 flex-shrink-0">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold" style={{ background: `${ev.c}22`, border: `1.5px solid ${ev.c}`, color: ev.c }}>
                {ev.year.slice(2)}
              </div>
  ...
  return (
    <div>
      ...
      <InfoBox color="red" title="🚨 Chlordécone : contamination persistante aux Antilles">
        Le chlordécone est un pesticide organochloré utilisé dans les bananeraies guadeloupéennes
        et martiniquaises de 1972 à 1993. Classé cancérigène possible (CIRC groupe 2B), il contamine
        durablement les sols et certaines sources d'eau. Des restrictions de consommation de végétaux
        et d'eau de source restent en vigueur dans certaines zones.
        <br /><br />
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-700">
            {[
              ...
              { t: 'La Réunion',  b: '98%', pc: '97%', r: 'Arsenic naturel (zones volc.)', bc: '#4ade80', pcc: '#4ade80' },
              { t: 'Guyane',      b: '89%', pc: '85%', r: 'Turbidité, microbiologie', bc: '#f59e0b', pcc: '#ef4444' },
              { t: 'Mayotte',     b: '84%', pc: '79%', r: 'Turbidité, nitrates, microbiologie', bc: '#ef4444', pcc: '#ef4444' },
              { t: 'Métropole',   b: '99%', pc: '98%', r: 'Nitrates localisés', bc: '#4ade80', pcc: '#4ade80' },
            ].map(row => (
              <tr key={row.t} className="hover:bg-slate-800/30">
                <td className="px-3 py-2 font-medium text-gray-200">{row.t}</td>
                <td className="px-3 py-2" style={{ color: row.bc }}>{row.b}</td>
                <td className="px-3 py-2" style={{ color: row.pcc }}>{row.pc}</td>
...
function TabTarifs() {
  return (
    <div>
      <SectionTitle icon={DollarSign}>Tarifs de l'eau dans les DOM : une inégalité structurelle</SectionTitle>
      <p className="text-gray-300 text-sm mb-6 leading-relaxed">
        Le prix de l'eau est fixé localement par chaque collectivité délégataire. Dans les DOM,
        la combinaison de réseaux dégradés, de faibles économies d'échelle et de coûts d'importation
        de matériaux engendre des tarifs systématiquement supérieurs à la moyenne métropolitaine.
        L'UFC-Que Choisir et la Cour des Comptes ont documenté ces écarts.
      </p>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <DataCard label="Prix moyen eau potable DOM" value="~3,80 €/m³" sub="abonnement inclus" highlight />
        <DataCard label="Prix moyen eau potable métropole" value="~2,10 €/m³" sub="abonnement inclus" />
        <DataCard label="Écart moyen DOM/métropole" value="+81%" sub="Source : UFC-QC 2023" />
        <DataCard label="Variation max. intra-DOM" value="x3" sub="entre organismes d'un même territoire" />
      </div>

      <InfoBox color="cyan" title="📊 Méthode tarifaire : comment lire une facture d'eau ?">
        La facture d'eau comprend deux parties :{' '}
        <strong>la part fixe (abonnement)</strong>, indépendante de la consommation, et{' '}
        <strong>la part variable (prix au m³)</strong>, proportionnelle à la consommation.
        En France, l'assainissement représente en moyenne 40–50 % de la facture totale.
        Les données SISPEA (DGALN) permettent de comparer les services eau+assainissement.
      </InfoBox>

      <SectionTitle icon={BarChart2}>Comparatif tarifaire eau+assainissement (prix TTC, 120 m³/an)</SectionTitle>
      <div className="overflow-x-auto mb-6">
        <table className="w-full text-sm">
          <thead className="bg-slate-800">
            <tr>
          ...
          <tbody className="divide-y divide-slate-700">
            {[
              { t: 'GDEG — Guadeloupe',        ep: '4,85 €', as: '2,45 €', ab: '9,75 €',  s: 'SISPEA 2023' },
              { t: 'Régie Basse-Terre — GP',   ep: '4,60 €', as: '1,90 €', ab: '9,20 €',  s: 'SISPEA 2023' },
              { t: 'ODYSSI — Martinique',       ep: '3,95 €', as: '2,20 €', ab: '8,50 €',  s: 'SISPEA 2023' },
              { t: 'CAP Nord — Martinique',     ep: '4,10 €', as: '2,35 €', ab: '8,90 €',  s: 'SISPEA 2023' },
              { t: 'SPL Eau Réunion — Nord',    ep: '2,80 €', as: '1,65 €', ab: '7,20 €',  s: 'SISPEA 2023' },
              { t: 'CISE Réunion — Ouest',      ep: '3,10 €', as: '1,80 €', ab: '7,80 €',  s: 'SISPEA 2023' },
              { t: 'Cayenne — Guyane',          ep: '3,50 €', as: '1,95 €', ab: '8,10 €',  s: 'SISPEA 2023' },
              { t: 'SMEG — Mayotte',            ep: '5,20 €', as: 'N/A',    ab: '12,40 €', s: 'ARS Mayotte 2023' },
              { t: 'Métropole (médiane)',        ep: '2,10 €', as: '1,55 €', ab: '6,50 €',  s: 'UFC-QC 2023' },
            ].map(row => (
              <tr key={row.t} className="hover:bg-slate-800/30">
      ...
      <InfoBox color="amber" title="⚠️ Données indicatives">
        Les tarifs présentés sont des exemples représentatifs issus du système SISPEA (DGALN) et
        des rapports ARS. Ils peuvent évoluer annuellement par délibération des assemblées locales.
        Les consommations réelles, la part redevances et les taxes locales peuvent modifier
      ...
      <Collapse title="Pourquoi l'eau est-elle si chère dans les DOM ?">
        ...
        <p className="mb-3">
          <strong>2. Pertes sur réseau :</strong> Un taux de fuite de 60% en Guadeloupe signifie
          que les coûts de production sont répartis sur une eau effectivement livrée deux fois moins
...
function TabInfrastructure() {
  return (
    <div>
      <SectionTitle icon={Wrench}>L'état des réseaux : un héritage de sous-investissement</SectionTitle>
      <p className="text-gray-300 text-sm mb-6 leading-relaxed">
        La Cour des Comptes, dans son rapport de 2020 sur les services d'eau dans les DOM, a
        dressé un constat sévère : des décennies de sous-investissement ont conduit à un état
        de délabrement avancé des réseaux, particulièrement en Guadeloupe. Le taux de renouvellement
        annuel des canalisations y est estimé à moins de 0,5 % contre 0,8 % recommandé en métropole.
      </p>

      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <DataCard label="Taux de fuite Guadeloupe" value="~60%" sub="vs 20% en métropole" highlight />
        ...
        <DataCard label="Linéaire réseau Guadeloupe" value="~4 200 km" sub="dont ~40% à rénover urgence" />
        <DataCard label="Âge moyen canalisations DOM" value="35–50 ans" sub="Source : Cour des Comptes 2020" />
        <DataCard label="Besoin d'investissement DOM" value="~2,5 Md€" sub="horizon 2030 — estimation État" />
      </div>
      ...
      <InfoBox color="red" title="🚨 Crise Guadeloupe : un réseau en urgence">
        ...
        plusieurs jours concernent régulièrement des quartiers entiers, notamment dans la zone
        de la Basse-Terre. En 2022, le gouvernement a déclenché une procédure d'urgence et débloqué
        40 M€ de crédits exceptionnels. Le diagnostic : canalisations amiante-ciment datant des
        années 1960-1970, joints défaillants, pompes hors service.
      </InfoBox>

      <SectionTitle icon={TrendingUp}>Taux de fuite comparés (pertes sur réseau)</SectionTitle>
      <div className="space-y-3 mb-6">
        {[
          { t: 'Guadeloupe',  v: 60, note: 'Situation critique, plan urgence déclenché',  c: '#ef4444' },
      ...
      <Collapse title="Plan de renouvellement des réseaux : ce qui est prévu">
        ...
        <p className="mb-3">
          Le Plan Eau 2023 a ajouté une enveloppe spécifique outre-mer de 50 M€ supplémentaires
          pour les réseaux les plus dégradés, avec une priorité pour la Guadeloupe et Mayotte.
        </p>
        <p>
          L'objectif national fixé par la DGALN est de ramener le taux de fuite sous les 30 %
          dans tous les DOM d'ici 2030, ce qui nécessite de tripler le rythme de renouvellement
          des canalisations.
        </p>
      </Collapse>
      ...
      <Collapse title="Assainissement : le parent pauvre de l'investissement">
        <p className="mb-3">
          L'assainissement (collecte et traitement des eaux usées) est encore plus sous-financé
          que l'eau potable dans les DOM. Des zones urbanisées importantes ne sont toujours pas
          raccordées à des systèmes d'assainissement collectif conformes.
        </p>
        <p>
          En Guyane, plus de 30 % de la population utilise encore des systèmes d'assainissement
          non collectifs (fosses septiques) dont l'état et le contrôle sont insuffisants.
          En Mayotte, la situation est encore plus précaire avec des rejets directs dans le
          milieu naturel dans certaines zones.
        </p>
      </Collapse>
    </div>
  );
...
function TabActeurs() {
  return (
    <div>
      ...
      <p className="text-gray-300 text-sm mb-6 leading-relaxed">
        La gestion de l'eau est une compétence <strong className="text-white">communale ou intercommunale</strong>{' '}
        en France. Les communes peuvent choisir la gestion en régie (service public direct) ou
        la délégation de service public (DSP) à un opérateur privé. Dans les DOM, les deux modes
        coexistent, avec une tendance à la consolidation intercommunale.
      </p>
        ...
        {[
          {
            ...
            color: '#22d3ee',
            operateurs: [
              { nom: 'GDEG (Générale des Eaux Guadeloupe)', type: 'Filiale Veolia', zone: 'Grande-Terre & Marie-Galante', contrat: 'DSP' },
              { nom: 'SMO Guadeloupe', type: 'Syndicat Mixte Ouvert', zone: 'Production principale', contrat: 'Coord. territoriale' },
              { nom: 'Régie Basse-Terre', type: 'Régie publique', zone: 'Secteur Basse-Terre', contrat: 'Régie directe' },
            ],
          },
          {
            ...
            color: '#818cf8',
            operateurs: [
              { nom: 'ODYSSI', type: 'Syndicat mixte intercommunal', zone: 'Centre & Sud', contrat: 'Service public' },
              { nom: 'CAP Nord', type: 'Communauté d\'agglo.', zone: 'Nord Martinique', contrat: 'Régie' },
              { nom: 'ODE Martinique', type: 'Office De l\'Eau', zone: 'Coordination régionale', contrat: 'Observatoire' },
            ],
          },
          {
            ...
            color: '#4ade80',
            operateurs: [
              { nom: 'SPL Eau de La Réunion', type: 'Société Publique Locale', zone: 'Nord & Est', contrat: 'SPL' },
              { nom: 'CISE Réunion (Suez)', type: 'Filiale Suez', zone: 'Ouest & Sud', contrat: 'DSP' },
              { nom: 'Office de l\'Eau Réunion', type: 'EP local', zone: 'Coordination régionale', contrat: 'Observatoire' },
            ],
          },
          {
            territoire: 'Guyane 🇬🇫',
            color: '#f59e0b',
            operateurs: [
              { nom: 'Eau et Électricité de Guyane (EEG)', type: 'Filiale EDF-SEEAG', zone: 'Cayenne + communes',  contrat: 'DSP' },
              { nom: 'Communes isolées', type: 'Régies locales', zone: 'Intérieur & fleuves', contrat: 'Régie' },
              { nom: 'ODE Guyane', type: 'Office De l\'Eau', zone: 'Coordination régionale', contrat: 'Observatoire' },
            ],
          },
          {
            territoire: 'Mayotte 🇾🇹',
            color: '#ef4444',
            operateurs: [
              { nom: 'SMEG (Sté Mahoraise d\'Eau et de Gaz)', type: 'Filiale Veolia', zone: 'Réseau principal', contrat: 'DSP' },
              { nom: 'Département de Mayotte', type: 'Collectivité', zone: 'Coordination & financement', contrat: 'Tutelle' },
            ],
          },
        ].map(({ territoire, color, operateurs }) => (
          <div key={territoire} className="bg-slate-800/50 rounded-xl overflow-hidden border border-slate-700/50">
            <div className="px-4 py-3 font-semibold text-sm" style={{ color, borderBottom: `1px solid ${color}33`, background: `${color}11` }}>
              {territoire}
            </div>
              ...
              {operateurs.map(op => (
                <div key={op.nom} className="px-4 py-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-slate-700 text-gray-400 flex-shrink-0">{op.contrat}</span>
                  </div>
                  <p className="text-xs text-gray-500 mt-0.5">{op.type} — {op.zone}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        {[
          { nom: 'ARS (Agences Régionales de Santé)', role: 'Contrôle de la qualité de l\'eau potable, analyses, autorisation des captages', couleur: '#22d3ee' },
          { nom: 'DGALN / SISPEA', role: 'Ministère de la Transition écologique — collecte des données tarifaires et techniques (SISPEA)', couleur: '#60a5fa' },
          { nom: 'Offices de l\'Eau DOM', role: 'Connaissance des ressources, soutien technique aux collectivités, observatoires territoriaux', couleur: '#818cf8' },
          { nom: 'BRGM', role: 'Bureau de Recherches Géologiques et Minières — étude des ressources souterraines, hydrogéologie', couleur: '#4ade80' },
          { nom: 'UFC-Que Choisir', role: 'Comparatif tarifaire annuel, alertes consommateurs, rapports sur la qualité des services', couleur: '#f59e0b' },
          { nom: 'Cour des Comptes', role: 'Contrôle de la gestion des services, rapports sur les finances des collectivités locales', couleur: '#ef4444' },
        ].map(a => (
          <div key={a.nom} className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
            <p className="text-sm font-semibold mb-1" style={{ color: a.couleur }}>{a.nom}</p>
  ...
  return (
    <div>
      ...
      <InfoBox color="red" title="🚨 Mayotte : état d'urgence hydrique structurel">
        Mayotte est le territoire français où l'accès à l'eau potable est le plus précaire.
        En 2024, une restriction d'eau sévère (alimentation 1 jour sur 2 dans certaines zones)
        a été maintenue pendant plusieurs semaines. Les causes sont multiples : infrastructure
        insuffisante, croissance démographique rapide (+3,8% par an), ressources en eau limitées.
      </InfoBox>

      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <DataCard label="Pop. sans accès eau potable réseau" value="~18%" sub="Source : ARS Mayotte 2023" highlight />
        <DataCard label="Taux de fuite réseau" value="~42%" sub="SISPEA 2022" />
        <DataCard label="Prix moyen eau potable SMEG" value="5,20 €/m³" sub="le plus élevé des DOM" />
        <DataCard label="Capacité stockage eau Mayotte" value="~7 jours" sub="en période de sécheresse" />
        <DataCard label="Besoins investissement urgents" value="~400 M€" sub="Estimation DGALN 2023" />
        <DataCard label="Croissance démographique" value="+3,8%/an" sub="pression sur les ressources" />
      </div>

      <Collapse title="La crise de 2024 : anatomie d'une rupture d'approvisionnement">
        <p className="mb-3">
          En 2023-2024, Mayotte a subi une crise hydrique majeure combinant :
        </p>
        <ul className="list-disc list-inside space-y-2 mb-3">
          <li>Une sécheresse exceptionnelle (précipitations inférieures de 40% à la normale)</li>
          <li>Des retards dans la construction de la deuxième retenue d'eau (Retenue de Dzoumogné)</li>
        ...
        <p>
          En réponse, l'État a déployé des camions-citernes, augmenté les transferts d'eau
          depuis la Réunion par bateau, et accéléré le calendrier des travaux d'urgence.
          Un plan d'investissement exceptionnel de 400 M€ a été annoncé pour 2024-2027.
        </p>
      </Collapse>
      ...
      <InfoBox color="amber" title="🌿 Guyane : 18% du territoire sans réseau public">
        La Guyane est le seul DROM où une fraction significative de la population n'a pas
        accès à un réseau public d'eau potable. Les communautés amérindiennes et bushinengué
        de l'intérieur et des fleuves frontaliers (Maroni, Oyapock) dépendent essentiellement
        de l'eau de pluie et des cours d'eau, sans traitement standardisé.
      </InfoBox>

      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <DataCard label="Pop. zones isolées sans réseau" value="~15 000" sub="Communes du fleuve" highlight />
        <DataCard label="Taux de turbidité eaux de surface" value="Élevé" sub="en saison des pluies" />
        <DataCard label="Distance Cayenne → St-Laurent" value="~250 km" sub="Accès routier seul, partiel" />
        <DataCard label="Nb. communes sans service eau" value="~8" sub="Source : SISPEA Guyane 2022" />
      </div>
      ...
      <Collapse title="Les défis spécifiques de la Guyane en matière d'eau">
        <p className="mb-3">
          ...
          amazonienne. Les communes de l'intérieur (Saint-Laurent-du-Maroni, Maripasoula, Papaichton,
          Grand-Santi…) sont soit non raccordées à un réseau national, soit desservies par des
          systèmes locaux insuffisants.
        </p>
        <p className="mb-3">
          La pollution au mercure des cours d'eau, due à l'orpaillage illégal (ASGMI), aggrave
          la situation dans certaines zones, rendant l'eau de rivière impropre à la consommation
          sans traitement spécialisé.
        </p>
        <p>
          Le Plan Eau 2023 prévoit un soutien spécifique aux communes de l'intérieur guyanais,
          avec la mise en place de stations de traitement autonomes et de citernes villageoises.
        </p>
      </Collapse>
    </div>
  );
    ...
    <div>
      ...
      <div className="space-y-3 mb-8">
        {[
          ...
          <div key={src.label} className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
      ...
      <div className="space-y-3 mb-6">
        {[
          {
            n: '1', title: 'Accélérer le renouvellement des réseaux',
            detail: 'Porter le taux annuel de renouvellement des canalisations de 0,3 % à 1,5 % dans les DOM les plus déficitaires. Financement : FEDER, dotations État, tarification progressive.',
            c: '#22d3ee',
          },
          {
            n: '2', title: 'Transparence tarifaire renforcée',
            detail: 'Obligation de publication annuelle des données SISPEA pour toutes les collectivités délégataires dans les DOM. Rapport comparatif public par ARS.',
            c: '#60a5fa',
          },
          {
            n: '3', title: 'Gratuité des premiers m³ vitaux',
            detail: 'Expérimentation d\'une allocation d\'eau gratuite (ex. 10 m³/mois par foyer) dans les territoires les plus pauvres (Mayotte, Guyane intérieure). Couverture par solidarité nationale.',
            c: '#4ade80',
          },
          {
            n: '4', title: 'Plan d\'urgence Mayotte et Guyane intérieure',
            detail: 'Plan décennal spécifique à ces deux territoires, avec gouvernance directe État-collectivité et objectifs chiffrés annuels de raccordement.',
            c: '#f59e0b',
          },
          {
            n: '5', title: 'Mutualisation régionale des expertises',
            detail: 'Créer un réseau interDOM des Offices de l\'eau, BRGM et ARS pour partager les bonnes pratiques et optimiser les investissements.',
            c: '#a78bfa',
          },
        ].map(r => (
          <div key={r.n} className="flex gap-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-black flex-shrink-0 mt-0.5" style={{ background: `${r.c}22`, border: `1.5px solid ${r.c}`, color: r.c }}>
              {r.n}
            </div>
            <div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <Helmet>
        ...
        <link rel="alternate" hrefLang="fr" href="https://teetee971.github.io/akiprisaye-web/enquete-eau" />
        <link rel="alternate" hrefLang="x-default" href="https://teetee971.github.io/akiprisaye-web/enquete-eau" />
      </Helmet>

      <div className="max-w-5xl mx-auto px-4 pb-16 pt-6">
        {/* Back navigation */}
        <div className="mb-4 flex flex-wrap gap-3">
          <Link to="/recherche-prix/eau"
        ...
        <div className="mb-6">
          ...
          >
            ...
            <h1 className="text-2xl sm:text-4xl font-black text-white drop-shadow leading-tight">
              💧 L'Eau dans les DOM<br />sous la loupe
            </h1>
            <p className="text-cyan-100 text-sm mt-2 drop-shadow max-w-2xl">
              ...
              L'enquête complète de l'Observatoire A KI PRI SA YÉ sur l'eau dans les territoires ultramarins.
            </p>
        ...
        {/* Key figures */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          <DataCard label="Écart tarifaire moyen DOM/Métropole" value="+81%" sub="Source : UFC-QC 2023" highlight />
        ...
        {/* Disclaimer */}
        <InfoBox color="amber" title="⚠️ Note méthodologique">
          Toutes les données sont issues de sources officielles publiques (ARS, SISPEA/DGALN, Offices de l'eau DOM,
          Cour des Comptes, UFC-Que Choisir). Les tarifs cités sont des exemples représentatifs —
          la tarification est fixée localement par chaque collectivité. Cette page vise à informer,
          pas à remplacer une consultation juridique ou technique.
        </InfoBox>
