  ...
  return (
    <a
      ...
      style={{
        ...
        color: '#fb923c', fontSize: '0.72rem', textDecoration: 'none',
  ...
  return (
    <article style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.7rem', color: '#f97316', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.5rem' }}>
        ...
        {lettre.indicateurs.length > 0 && (
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', paddingTop: '0.75rem', borderTop: '1px solid rgba(249,115,22,0.2)' }}>
            {lettre.indicateurs.map((ind, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: '1.2rem', fontWeight: 900, color: '#fdba74', lineHeight: 1 }}>{ind.valeur}</div>
                <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginTop: '0.2rem', lineHeight: 1.35 }}>{ind.label}</div>
                <div style={{ fontSize: '0.62rem', color: '#f97316', marginTop: '0.1rem', fontWeight: 600 }}>{ind.territoire}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {lettre.sections.map((s, i) => (
        <div key={i} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.6rem', flexWrap: 'wrap' }}>
            ...
            <span style={{
              fontSize: '0.67rem', color: '#f97316', fontWeight: 700,
              background: 'rgba(249,115,22,0.1)', padding: '2px 9px',
              borderRadius: 20, border: '1px solid rgba(249,115,22,0.25)', whiteSpace: 'nowrap',
            }}>
              {s.territoire}
            </span>
          </div>
            ...
            <div style={{ marginTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              {lettre.sourcesArticles.map((a, i) => (
                <div key={i} style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                  ...
                  <a href={a.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.72rem', color: '#fb923c', textDecoration: 'none', lineHeight: 1.45 }}>
                    {a.title}
                  </a>
                </div>
              ))}
  ...
  return (
    <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
      ...
      <p style={{ margin: '0 0 1.5rem', fontSize: '0.88rem', color: '#64748b', lineHeight: 1.7, maxWidth: 420, marginLeft: 'auto', marginRight: 'auto' }}>
        L'intelligence artificielle génère automatiquement un briefing chaque <strong style={{ color: '#e2e8f0' }}>matin à 7h UTC</strong>{' '}
        à partir des actualités des territoires ultramarins (La1ère, Outremers360°, etc.).
        Revenez demain matin pour lire le premier numéro !
      </p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center', fontSize: '0.82rem', color: '#475569' }}>
        {['📡 Collecte automatique des actualités DOM/COM', '🧠 Analyse et rédaction par GPT-4o-mini', '💾 Publication dans Firestore chaque matin à 7h UTC', '📱 Accessible sur cette page et sur l\'accueil'].map((s) => (
          <span key={s}>{s}</span>
        ))}
      </div>
    </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', maxHeight: '60vh', overflowY: 'auto' }}>
                {archive.map((l) => (
                  <button
                    ...
                    style={{
                      background: (current?.dayId === l.dayId || selectedDayId === l.dayId) ? 'rgba(249,115,22,0.15)' : 'transparent',
                  ...
                  >
                    <div style={{ fontSize: '0.65rem', color: '#f97316', fontWeight: 700 }}>{formatDayId(l.dayId)}</div>
                    <div style={{ fontSize: '0.72rem', color: (current?.dayId === l.dayId) ? '#e2e8f0' : '#94a3b8', lineHeight: 1.3, marginTop: '0.15rem' }}>
