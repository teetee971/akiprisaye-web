...
const ENDPOINTS: Endpoint[] = [
  {
    ...
    path: '/api/v1/prices',
    description:
      'Liste des prix par territoire. Retourne les prix les plus récents pour chaque produit et enseigne.',
    params: [
      {
        name: 'territory',
        in: 'query',
        required: false,
        type: 'string',
        description: 'Code territoire (ex: martinique, guadeloupe)',
      },
      {
        name: 'category',
        in: 'query',
      ...
      },
      {
        ...
        required: false,
        type: 'string',
        description: 'Clé API pour quota étendu',
      },
    ],
  ...
  },
  {
    ...
    params: [],
    exampleResponse: JSON.stringify(
      { status: 'ok', version: '1.0', timestamp: 1705483200000 },
      null,
      2
    ),
  },
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <span
            className={`text-xs font-bold px-2 py-1 rounded font-mono ${METHOD_COLORS[endpoint.method]}`}
          >
            {endpoint.method}
          </span>
                  ...
                  <thead>
                    <tr className="border-b border-gray-200 dark:border-gray-700">
                      {['Nom', 'Dans', 'Requis', 'Type', 'Description'].map((h) => (
                        <th
                          key={h}
                          className="text-left py-1.5 px-2 text-xs text-gray-500 dark:text-gray-400 font-medium"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {endpoint.params.map((p) => (
                      <tr key={p.name} className="border-b border-gray-100 dark:border-gray-800">
                        <td className="py-1.5 px-2 font-mono text-xs text-gray-900 dark:text-white">
                          {p.name}
                        </td>
                        ...
                        <td className="py-1.5 px-2 font-mono text-xs text-blue-600 dark:text-blue-400">
                          {p.type}
                        </td>
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-5">
        ...
        <p className="text-sm text-blue-800 dark:text-blue-400 mb-3">
          ...
          <code className="font-mono bg-blue-100 dark:bg-blue-800 px-1 rounded">x-api-key</code>.
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
        ...
        <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
          Les headers <code className="font-mono">X-RateLimit-Limit</code>,{' '}
          <code className="font-mono">X-RateLimit-Remaining</code> et{' '}
