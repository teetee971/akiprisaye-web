...
const baseComparisons: ProductComparison[] = [
  ...
  },
  {
    ...
    category: 'Viande',
    prices: [
      { store: 'Carrefour', price: 8.9 },
      { store: 'E.Leclerc', price: 8.2 },
      { store: 'Super U', price: 8.55 },
    ],
  },
  {
    name: 'Pâtes 500 g',
    ...
    prices: [
      ...
      { store: 'E.Leclerc', price: 1.25 },
      { store: 'Super U', price: 1.35 },
    ],
  },
  {
    name: 'Bananes 1 kg',
    ...
    prices: [
      { store: 'Carrefour', price: 1.8 },
      { store: 'E.Leclerc', price: 1.6 },
      { store: 'Super U', price: 1.7 },
    ],
  },
  {
    name: 'Pain de mie',
    ...
    prices: [
      { store: 'Carrefour', price: 2.3 },
      { store: 'E.Leclerc', price: 2.05 },
      { store: 'Super U', price: 2.2 },
    ],
  },
  {
    name: 'Sucre 1 kg',
    category: 'Épicerie',
    prices: [
      { store: 'Carrefour', price: 1.55 },
      { store: 'E.Leclerc', price: 1.4 },
      { store: 'Super U', price: 1.48 },
    ],
  },
];
...
export default function Comparateurs() {
  ...
  const totalMin = comparisons.reduce(
    (sum, p) => sum + Math.min(...p.prices.map((s) => s.price)),
    0
  );
  const totalMax = comparisons.reduce(
    (sum, p) => sum + Math.max(...p.prices.map((s) => s.price)),
    0
  );
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="mb-6 animate-fade-in">
            ...
            >
              ...
              <p className="text-slate-200 text-sm drop-shadow">
                Données indicatives – territoires ultramarins ·{' '}
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 mb-6">
            ...
            <p className="text-xs text-white/40 mt-3">
              📌 Données indicatives à usage informatif. Prix moyens observés sur les enseignes
              locales.
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {stores.map((store) => {
                        ...
                        if (!entry)
                          return (
                            <td key={store} className="px-4 py-3 text-white/30">
                              —
                            </td>
                          );
                        ...
                        return (
                          <td
                            key={store}
                            className={`px-4 py-3 font-semibold ${isBest ? 'text-emerald-300' : 'text-white'}`}
                          >
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-white/40">
              Prix estimés. Pour des prix en temps réel, utilisez le comparateur avancé.
            </p>
            <div className="flex gap-3">
              <Link
                to="/scanner?mode=ticket"
