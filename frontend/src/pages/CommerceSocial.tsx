...
import { Share2, Star, Users, PlusCircle, Trash2, Copy, User, Edit3, X, Check } from 'lucide-react';
...
export default function CommerceSocial() {
  ...
  function handleAddItem(listId: string) {
    ...
    if (!item) return;
    const updated = lists.map((l) => (l.id === listId ? { ...l, items: [...l.items, item] } : l));
    setLists(updated);
    saveLists(updated);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {profileTerritory?.name ?? 'Territoire inconnu'}
                </p>
              </div>
                ...
                <div key={list.id} className="bg-white border border-gray-200 rounded-xl p-4">
                  ...
                  <div className="flex gap-2 mt-2">
                    <input
                      ...
                      onChange={(e) =>
                        setNewItem((prev) => ({ ...prev, [list.id]: e.target.value }))
                      }
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setRecForm((f) => ({ ...f, territory: e.target.value as TerritoryCode }))
                  }
                  className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        ...
                        {rec.price && (
                          <span className="ml-2 text-pink-700 font-semibold">{rec.price}</span>
                        )}
                      </p>
                      {rec.comment && (
                        <p className="text-xs text-gray-600 mt-1 italic">{rec.comment}</p>
                      )}
                    </div>
                  </div>
                );
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDraftProfile((p) => ({ ...p, territory: e.target.value as TerritoryCode }))
                  }
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-pink-500"
                >
                  {activeTerritories.map((t) => (
