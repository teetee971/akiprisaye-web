...
export default function FraudDetection() {
  ...
  const filtered = useMemo(
    () =>
      allAnomalies.filter(
        (a) =>
          (territory === 'Tous' || a.territory === territory) &&
          a.mlScore >= minScore,
      ),
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 py-8 space-y-8'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        {[
          { label: 'Alertes cette semaine', value: stats.thisWeek, icon: AlertTriangle, color: 'text-blue-500' },
          { label: 'Alertes critiques', value: stats.critical, icon: AlertTriangle, color: 'text-red-500' },
          { label: 'Résolues', value: stats.resolvedCount, icon: CheckCircle, color: 'text-green-500' },
        ...
        <div className='space-y-3'>
          {top5.map((a) => (
            ...
            >
              ...
              <div className='flex items-center gap-3'>
                <span className={`text-xs font-bold px-2 py-1 rounded-full ${scoreBadge(a.mlScore)}`}>
                  {a.mlScore}
                </span>
                {resolved.has(a.id) ? (
          ...
          <table className='w-full text-sm'>
            ...
            <tbody>
              {filtered.map((a) => (
                ...
                >
                  <td className='py-2 px-3 text-gray-900 dark:text-white font-medium'>{a.productName}</td>
                  <td className='py-2 px-3 text-gray-600 dark:text-gray-300'>{a.store}</td>
                  <td className='py-2 px-3 text-gray-900 dark:text-white'>{a.price.toFixed(2)} €</td>
                  <td className={`py-2 px-3 font-medium ${a.deviation > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                    {a.deviation > 0 ? '+' : ''}{a.deviation.toFixed(1)}%
                  </td>
                  <td className='py-2 px-3'>
                    <span className={`font-bold px-2 py-0.5 rounded-full text-xs ${scoreBadge(a.mlScore)}`}>
                      {a.mlScore}
                    </span>
                  </td>
                  <td className='py-2 px-3'>
                    <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${scoreBadge(a.mlScore)}`}>
                      {a.status}
                    </span>
                  </td>
                </tr>
