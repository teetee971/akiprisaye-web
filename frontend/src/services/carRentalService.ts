/**
 * Car Rental Comparison Service v1.0.0
 *
 * - Lecture seule des données
 * - Comparaison multi-agences par territoire et catégorie
 * - "Observer, pas vendre" : transparence sur les écarts, pas d'affiliation
 */
...
/**
 * Compare les prix de location de voitures pour un territoire et une catégorie donnés
 */
...
function calculateAggregation(
  ...
  return {
    ...
    internationalAgencyCount: prices.filter((p) => !p.isLocalAgency).length,
  };
}
...
function rankAgencies(prices: CarRentalPricePoint[], average: number): CarRentalRanking[] {
  ...
  return sorted.map((price, idx) => {
    ...
    let priceCategory: CarRentalRanking['priceCategory'];
    if (idx === 0) {
...
function buildMetadata(prices: CarRentalPricePoint[]): CarRentalMetadata {
  const dates = prices.map((p) => p.observationDate).sort();
  return {
    methodology: 'v1.0.0',
...
/** Catégories avec leur libellé français */
