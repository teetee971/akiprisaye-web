...
async function loadTerritoryData(territory?: string): Promise<EnhancedPriceData> {
  ...
  const data = await liveApiFetchJson<EnhancedPriceData>(
    `/prices/enhanced${query ? `?${query}` : ''}`,
    {
...
function processSearchFilters(
  ...
  results.sort((a, b) => {
    ...
    switch (sortBy) {
      ...
      case 'price': {
        const pricesA = a.product.prices.filter(
          (p) => !filters.territory || p.territory === filters.territory
        );
        const pricesB = b.product.prices.filter(
          (p) => !filters.territory || p.territory === filters.territory
        );

        const minPriceA = pricesA.length > 0 ? Math.min(...pricesA.map((p) => p.price)) : Infinity;
        const minPriceB = pricesB.length > 0 ? Math.min(...pricesB.map((p) => p.price)) : Infinity;
      ...
      case 'reliability': {
        const avgReliabilityA =
          a.product.prices.reduce((sum, p) => sum + p.reliability.score, 0) /
          a.product.prices.length;
        const avgReliabilityB =
          b.product.prices.reduce((sum, p) => sum + p.reliability.score, 0) /
          b.product.prices.length;
...
export async function comparePrices(
  ...
  try {
    ...
    const priceRangePercentage = cheapestPrice > 0 ? (priceRange / cheapestPrice) * 100 : 0;
    ...
    const avgReliability =
      sortedPrices.reduce((sum, p) => sum + p.reliability.score, 0) / sortedPrices.length;
