 ...
 */
function extractNutritionalValues(
  ocrText: string
): ProductPhotoAnalysisResult['nutritionalValues'] {
  ...
  const fatMatch = ocrText.match(
    /(?:matières?\s+grasses?|lipides?|fat)[\s:]*(\d+(?:[.,]\d+)?)\s*g/i
  );
  if (fatMatch) values.fat = parseFloat(fatMatch[1].replace(',', '.'));

  const satFatMatch = ocrText.match(
    /(?:acides?\s+gras\s+saturés?|saturated\s+fat)[\s:]*(\d+(?:[.,]\d+)?)\s*g/i
  );
 ...
 */
function calculateOCRQuality(
  ocrText: string,
  confidence: number
): ProductPhotoAnalysisResult['ocrQuality'] {
...
export async function analyzeProductPhoto(
  ...
  try {
    ...
    if (detectedEAN) {
      try {
        ...
        if (productLookup.success && productLookup.product) {
          ...
          const dossierResponse = await getProductDossier({
            ean: detectedEAN,
            territory:
              (options.territoire as unknown as import('../types/PriceObservation').TerritoryCode) ||
              undefined,
      ...
      warnings.push(
        'Code EAN non détecté. Analyse basée sur les informations visibles uniquement.'
      );
    }
