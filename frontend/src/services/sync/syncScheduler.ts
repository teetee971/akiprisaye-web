...
export function getSchedulerConfig(): SyncSchedulerConfig {
  try {
    const config = safeLocalStorage.getJSON<SyncSchedulerConfig>(CONFIG_KEY, {} as SyncSchedulerConfig);
...
export function updateScheduledJob(
  ...
  if (index !== -1) {
    jobs[index] = {
      ...(jobs[index] ?? { id: `job-${Date.now()}`, name: 'Job' }),
      ...(jobs[index]?.name ? {} : { name: 'Job' }),
    };
