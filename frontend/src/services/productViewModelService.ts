...
export function toProductViewModel(product: ProductResult): ProductViewModel {
  ...
  const statusColor = 
    product.status === 'confirmé' ? 'green' :
    product.status === 'partiel' ? 'yellow' :
    'gray';
  ...
  // Format date
  const dateObservation = new Date(product.traceability.dateObservation)
...
export function hasCompleteInfo(viewModel: ProductViewModel): boolean {
  return viewModel.status === 'confirmé' && 
