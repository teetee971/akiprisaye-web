...
export async function getProductHistory(
  ...
  try {
    ...
    if (request.startDate) {
      ...
      reformulations = reformulations.filter(
        (r) => new Date(r.detectedAt).getTime() >= startTimestamp
      );
    }
    ...
    if (request.endDate) {
      ...
      reformulations = reformulations.filter(
        (r) => new Date(r.detectedAt).getTime() <= endTimestamp
      );
    }
