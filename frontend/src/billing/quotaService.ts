...
const keyForDate = (date = new Date()) => `akiprisaye_quota_refresh_${date.toISOString().slice(0, 10).replace(/-/g, '')}`;
...
export function assertQuotaOrThrow(quotaName: 'refreshPerDay', limit: number, date = new Date()) {
  ...
  if (used >= limit) {
    emitUpgradePrompt({ quotaName, message: 'Quota quotidien atteint. Passez à PRO pour continuer.' });
