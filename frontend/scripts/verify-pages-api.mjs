...
const strictApiVerification = process.env.CI_STRICT_API_VERIFY === '1';
...
async function main() {
  try {
    ...
    if (isNetworkEnvironmentError(error)) {
      if (strictApiVerification) {
        console.error(`API verification failed in strict mode for ${baseUrl}: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
