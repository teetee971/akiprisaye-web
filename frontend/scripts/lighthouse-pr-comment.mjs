...
// ─── Read optional baseline ───────────────────────────────────────────────────

let baseline = null;
const baselinePath = process.env.BASELINE_SCORES_PATH;
...
const rows = [
  { label: 'Performance',   key: 'performance',   threshold: 80  },
  { label: 'Accessibilité', key: 'accessibility', threshold: 90  },
  { label: 'SEO',           key: 'seo',           threshold: 80  },
  { label: 'Best Practices',key: 'bestPractices', threshold: null },
];

const tableHeader = '| Métrique | Score | Seuil | Statut |';
const tableSep    = '|---|---|---|---|';
const tableRows   = rows.map(({ label, key, threshold }) => {
  const score  = current[key];
  const base   = baseline?.[key] ?? null;
  const status = scoreIcon(score, threshold);
  const seuil  = threshold != null ? `≥ ${threshold}` : '—';
  const delta  = deltaStr(score, base);
  return `| **${label}** | ${score} / 100${delta} | ${seuil} | ${status} |`;
});
...
async function postOrUpdateComment() {
  ...
  if (existing) {
    ...
    console.log(`✅ Commentaire Lighthouse mis à jour (PR #${prNumber}, comment #${existing.id})`);
  } else {
    // Create new comment
    ...
    console.log(`✅ Commentaire Lighthouse créé (PR #${prNumber}, comment #${created.id})`);
  }
}
