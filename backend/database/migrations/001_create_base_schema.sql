...
function getPaypalSubscriptionId(event: PayPalWebhookEvent): string | null {
  // PayPal peut mettre l’ID de souscription selon les event_type dans des champs différents.
  const candidateIds = [event.resource?.id, event.resource?.billing_agreement_id, event.resource?.subscription_id];
  return (
    candidateIds.find((candidate): candidate is string => typeof candidate === 'string' && candidate.startsWith('I-')) ??
    null
  );
}
 ...
 * - Can be used by "resync-first" flows.
 */
...
export async function handleRequest(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/v1/webhooks/paypal') {
      ...
      // On garde 200 pour éviter des retry storms côté PayPal sur payloads foireux,
      // mais on loggue + on n’écrit rien de critique.
      ...
      // ✅ Enregistrement idempotent dans paypal_webhooks (INSERT OR IGNORE)
      const isNewEvent = await recordPayPalWebhookIfNew(env.PRICE_DB, {
        id: event.id,
        eventType: event.event_type,
      ...
      if (!isNewEvent) {
        ...
        console.log('paypal_webhook_duplicate_resynced', {
          eventId,
          eventType,
          paypalSubscriptionId: subscriptionId ?? 'unknown',
        });
    ...
    if (
      ...
      if (request.method === 'POST' && url.pathname === '/v1/admin/import/csv') {
        ...
        return withCors(
          adminJson({ status: 'queued', jobId: queued.jobId, filename: queued.filename }, 202),
          origin,
          env,
        );
      }
