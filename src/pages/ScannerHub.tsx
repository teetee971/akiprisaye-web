...
const DEFAULT_LOOKUP_TERRITORY = 'martinique' as const;
const SCAN_COOLDOWN_MS = 900;
const SAME_CODE_LOCK_MS = 2500;
...
async function resolveBarcode(barcode: string): Promise<ResolvedProduct | null> {
  ...
  const territoryFromQuery = searchParams?.get('territoire')?.trim().toLowerCase();

  const result = await lookupProductByEan(barcode, {
    territoire: territoryFromQuery || DEFAULT_LOOKUP_TERRITORY,
        ...
        const loop = async () => {
          ...
          try {
            ...
            if (import.meta.env.DEV) {
              console.warn('[ScannerHub] Erreur détection frame:', error);
            }
          }
...
export default function ScannerHub() {
  ...
  const isDuplicateRecently = useCallback((code: string) => {
    ...
    if (now - last.at < SCAN_COOLDOWN_MS) return true;
    if (code === last.code && now - last.at < SAME_CODE_LOCK_MS) return true;
  ...
  const upsertTopLoadingItem = useCallback((barcode: string) => {
    ...
    setResults((prev) => [item, ...prev].slice(0, MAX_SCAN_HISTORY_ITEMS));
    return item.id;
  }, []);
