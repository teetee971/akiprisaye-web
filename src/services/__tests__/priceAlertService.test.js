...
describe('Price Alert Service', () => {
  describe('detectPriceDrop', () => {
    it('should detect a price drop', () => {
      const result = detectPriceDrop(10.00, 8.00, DEFAULT_ALERT_PREFERENCES);
      expect(result).not.toBeNull();
      expect(result.triggered).toBe(true);
      expect(result.absoluteChange).toBe(-2.00);
      expect(result.percentageChange).toBe(-20);
    });

    it('should not trigger when price increases', () => {
      const result = detectPriceDrop(10.00, 12.00, DEFAULT_ALERT_PREFERENCES);
      expect(result).toBeNull();
    });

    it('should not trigger when price stays the same', () => {
      const result = detectPriceDrop(10.00, 10.00, DEFAULT_ALERT_PREFERENCES);
      expect(result).toBeNull();
    });

    it('should not trigger when disabled', () => {
      const preferences = { ...DEFAULT_ALERT_PREFERENCES, priceDropEnabled: false };
      const result = detectPriceDrop(10.00, 8.00, preferences);
      expect(result).toBeNull();
    });
    ...
    it('should calculate severity correctly for medium drop', () => {
      const result = detectPriceDrop(10.00, 9.20, DEFAULT_ALERT_PREFERENCES);
      expect(result.severity).toBe('medium'); // -8% is >= 5% but < 10%
    });

    it('should calculate severity correctly for low drop', () => {
      const result = detectPriceDrop(10.00, 9.60, DEFAULT_ALERT_PREFERENCES);
  ...
  describe('detectPriceIncrease', () => {
    it('should detect price increase exceeding percentage threshold', () => {
      const result = detectPriceIncrease(10.00, 11.00, DEFAULT_ALERT_PREFERENCES);
      expect(result).not.toBeNull();
      expect(result.triggered).toBe(true);
      expect(result.absoluteChange).toBe(1.00);
      expect(result.percentageChange).toBe(10);
    });

    it('should detect price increase exceeding absolute threshold', () => {
      const result = detectPriceIncrease(10.00, 10.60, DEFAULT_ALERT_PREFERENCES);
      expect(result).not.toBeNull();
      expect(result.triggered).toBe(true);
      expect(result.absoluteChange).toBeCloseTo(0.60, 2);
    });

    it('should not trigger when below both thresholds', () => {
      const result = detectPriceIncrease(10.00, 10.30, DEFAULT_ALERT_PREFERENCES);
      expect(result).toBeNull(); // +3% and +0.30€ both below thresholds
    });

    it('should not trigger when price decreases', () => {
      const result = detectPriceIncrease(10.00, 9.00, DEFAULT_ALERT_PREFERENCES);
      expect(result).toBeNull();
    });

    it('should not trigger when disabled', () => {
      const preferences = { ...DEFAULT_ALERT_PREFERENCES, priceIncreaseEnabled: false };
      const result = detectPriceIncrease(10.00, 12.00, preferences);
      expect(result).toBeNull();
    });

    it('should respect custom percentage threshold', () => {
      const preferences = { ...DEFAULT_ALERT_PREFERENCES, increasePercentageThreshold: 10, increaseAbsoluteThreshold: 1.00 };
      const result = detectPriceIncrease(10.00, 10.80, preferences);
      expect(result).toBeNull(); // +8% below 10% threshold AND +0.80€ below 1.00€ threshold
    });

    it('should respect custom absolute threshold', () => {
      const preferences = { ...DEFAULT_ALERT_PREFERENCES, increasePercentageThreshold: 20, increaseAbsoluteThreshold: 1.00 };
      const result = detectPriceIncrease(10.00, 10.80, preferences);
    ...
    it('should trigger when exceeding only percentage threshold', () => {
      const preferences = { ...DEFAULT_ALERT_PREFERENCES, increasePercentageThreshold: 5, increaseAbsoluteThreshold: 2.00 };
      const result = detectPriceIncrease(10.00, 10.60, preferences);
    ...
    it('should trigger when exceeding only absolute threshold', () => {
      const preferences = { ...DEFAULT_ALERT_PREFERENCES, increasePercentageThreshold: 20, increaseAbsoluteThreshold: 0.30 };
      const result = detectPriceIncrease(10.00, 10.40, preferences);
      expect(result).not.toBeNull(); // +0.40€ exceeds 0.30€ threshold even though +4% is below 20%
      expect(result.absoluteChange).toBeCloseTo(0.40, 2);
    });
  });
  ...
  describe('detectShrinkflation', () => {
    it('should detect shrinkflation (quantity reduction with price increase)', () => {
      const result = detectShrinkflation(
        10.00, // previous price
        10.00, // current price (same)
    ...
    it('should not trigger when disabled', () => {
      ...
      const result = detectShrinkflation(10.00, 10.00, 1000, 900, preferences);
      expect(result).toBeNull();
    });
  });
  ...
  describe('Alert Severity Calculation', () => {
    ...
    it('should classify medium severity for 5-10% change', () => {
      const result = detectPriceIncrease(10.00, 10.70, DEFAULT_ALERT_PREFERENCES);
      expect(result.severity).toBe('medium');
    });

    it('should classify severity based on percentage change', () => {
      const result = detectPriceIncrease(10.00, 10.51, DEFAULT_ALERT_PREFERENCES);
  ...
  describe('Edge Cases', () => {
    it('should handle very small price changes', () => {
      const result = detectPriceIncrease(1.00, 1.01, DEFAULT_ALERT_PREFERENCES);
      expect(result).toBeNull(); // +1% below threshold, +0.01€ below threshold
    });

    it('should handle large price changes', () => {
      const result = detectPriceIncrease(10.00, 50.00, DEFAULT_ALERT_PREFERENCES);
      expect(result).not.toBeNull();
    ...
    it('should handle zero previous price gracefully', () => {
      ...
      expect(() => {
        detectPriceIncrease(0, 10.00, DEFAULT_ALERT_PREFERENCES);
      }).not.toThrow();
    });
  });
});
